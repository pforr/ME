...
public class HoGiaDinhAction extends MVCPortlet {
	 ...
	 * renderResponse) throws IOException, PortletException { User user = null;
	 * UserMapping loginUserMapping = new UserMappingImpl(); String maTinh =
	 * ParamUtil.getString(renderRequest, VeSinhGiaDinhTerms.MA_TINH); String
	 * maHuyen = ParamUtil.getString(renderRequest,
	 * VeSinhGiaDinhTerms.MA_HUYEN); String maXa =
	 * ParamUtil.getString(renderRequest, VeSinhGiaDinhTerms.MA_XA); String
	 * loaiNhaTieu = ParamUtil.getString(renderRequest,
	 * VeSinhGiaDinhTerms.LOAI_NHA_TIEU); String trangThai =
	 * ParamUtil.getString(renderRequest, VeSinhGiaDinhTerms.TRANG_THAI); String
	 * namBaoCao = ParamUtil.getString(renderRequest,
	 * VeSinhGiaDinhTerms.NAM_BAO_CAO); List<DATAITEM> listTinh = new
	 * ArrayList<DATAITEM>(); List<DATAITEM> listHuyen = new
	 * ArrayList<DATAITEM>(); List<DATAITEM> listXa = new ArrayList<DATAITEM>();
	 * boolean isSupperAdmin = renderRequest.isUserInRole("Administrator");
	 * 
	 * try { user = PortalUtil.getUser(renderRequest); loginUserMapping =
	 * UserMappingBusiness.getUserMapping(user);
	 * 
	 * listTinh = DiaBanQuanLyUtils.getTinh(user, isSupperAdmin,
	 * QueryUtil.ALL_POS, QueryUtil.ALL_POS);
	 * 
	 * listHuyen = DiaBanQuanLyUtils.getHuyen(user, maTinh, 0l, isSupperAdmin,
	 * QueryUtil.ALL_POS, QueryUtil.ALL_POS);
	 * 
	 ...
	 * e.printStackTrace(); } catch (SystemException e) { // TODO Auto-generated
	 ...
	 * renderRequest.setAttribute(VeSinhGiaDinhTerms.LOAI_NHA_TIEU,
	 * loaiNhaTieu); renderRequest.setAttribute(VeSinhGiaDinhTerms.TRANG_THAI,
	 * trangThai); renderRequest.setAttribute(VeSinhGiaDinhTerms.NAM_BAO_CAO,
	 * namBaoCao);
	 * 
	...
	public void render(RenderRequest request, RenderResponse response)
		...
		String maTinh = ParamUtil
				.getString(request, VeSinhGiaDinhTerms.MA_TINH);
		String maHuyen = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.MA_HUYEN);
		String maXa = ParamUtil.getString(request, VeSinhGiaDinhTerms.MA_XA);
		String loaiNhaTieu = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.LOAI_NHA_TIEU);
		String id = ParamUtil.getString(request, VeSinhGiaDinhTerms.ID);
		String trangThai = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.TRANG_THAI);

		String namBaoCao = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.NAM_BAO_CAO);
		String namBaoCaoVstx = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.NAM_BAO_CAO + "vstx");

		String danhGiaKiemDem = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.DANH_GIA_KIEM_DEM);
		String ngayTao = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.NGAY_TAO);
		String ngayKiemDem = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.NGAY_KIEM_DEM);
		String giamSatVien = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.GIAM_SAT_VIEN);
		String kiemDemVien = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.KIEM_DEM_VIEN);
		String thonXom = ParamUtil.getString(request,
				VeSinhGiaDinhTerms.THON_XOM);
		...
		try {
			...
			if (Validator.isNotNull(id)) {
				veSinhGiaDinh = HoGiaDinhBussiness.fetchVeSinhGiaDinh(Integer
						.valueOf(id));
		...
		request.setAttribute(VeSinhGiaDinhTerms.NAM_BAO_CAO + "vstx",
				namBaoCaoVstx);
		...
		request.setAttribute(VeSinhGiaDinhTerms.DANH_GIA_KIEM_DEM,
				danhGiaKiemDem);
		request.setAttribute(VeSinhGiaDinhTerms.NGAY_TAO, ngayTao);
		request.setAttribute(VeSinhGiaDinhTerms.NGAY_KIEM_DEM, ngayKiemDem);
		request.setAttribute(VeSinhGiaDinhTerms.GIAM_SAT_VIEN, giamSatVien);
		request.setAttribute(VeSinhGiaDinhTerms.KIEM_DEM_VIEN, kiemDemVien);
		request.setAttribute(VeSinhGiaDinhTerms.THON_XOM, thonXom);
		super.render(request, response);
	}

	public void addHoGiaDinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		String messageKey = MessageErrors.ADD_HO_GIA_DINH_MESSAGE;
		long idReturn = 0;
		VeSinhGiaDinhTerms veSinhHoGiaDinhTerm = new VeSinhGiaDinhTerms(
				actionRequest);
		String add = "";
		...
		try {
			...
			if (add.equals(Constants.ADD)) {
				long id = 0;
				id = CounterLocalServiceUtil.increment(VeSinhGiaDinh.class
						.getName());
				hoGiaDinhEtt = VeSinhGiaDinhLocalServiceUtil
						.createVeSinhGiaDinh(Integer.valueOf(id + ""));
				hoGiaDinhEtt.setIdNguoiTao(Integer.valueOf(String
						.valueOf(userId)));
				hoGiaDinhEtt.setNgayTao(new Date());
				messageKey = MessageSuccess.ADD_HO_GIA_DINH_MESSAGE;
			} else {
				hoGiaDinhEtt = VeSinhGiaDinhLocalServiceUtil
						.fetchVeSinhGiaDinh(veSinhHoGiaDinhTerm.getId());
				messageKey = MessageSuccess.EDIT_HO_GIA_DINH_MESSAGE;
				idReturn = hoGiaDinhEtt.getId();
			}

			hoGiaDinhEtt.setGioiTinhChuHo(veSinhHoGiaDinhTerm
					.getGioiTinhChuHo());
			...
			hoGiaDinhEtt.setThanhPhanHoGD(veSinhHoGiaDinhTerm
					.getThanhPhanHoGD());

			hoGiaDinhEtt.setLoaiNhaTieu(veSinhHoGiaDinhTerm.getLoaiNhaTieu());
			hoGiaDinhEtt.setSoNguoiTrongHo(veSinhHoGiaDinhTerm
					.getSoNguoiTrongHo());
	...
	public void addDanhGiaLaiHoGiaDinh(ActionRequest actionRequest,
			ActionResponse actionResponse) {
		VeSinhGiaDinhTerms veSinhGiaDinhTerm = new VeSinhGiaDinhTerms(
				actionRequest);
		...
		try {
			...
			if (add.equals(Constants.ADD)) {
				...
				veSinhGiaDinhEtt = VeSinhGiaDinhLocalServiceUtil
						.fetchVeSinhGiaDinh(veSinhGiaDinhTerm.getId());
			}
	...
	public void deleteHoGiaDinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
	...
	public void verifiedDocumentData(ActionRequest actionRequest,
		...
		try {
			...
			int typeImport = ParamUtil.getInteger(actionRequest,
					VeSinhGiaDinhTerms.TYPE_IMPORT);
				...
				while (rowIterator.hasNext() && isNext > 0) {
					...
					while (cellIterator.hasNext() && isNext > 0) {
						...
						case 6:
							cell.setCellType(Cell.CELL_TYPE_STRING);
							if (typeImport == 1) {
								ett.setKhongCoNhaTieu(cell.getStringCellValue()
										.trim().length());
							} else {
						...
						case 7:
							cell.setCellType(Cell.CELL_TYPE_STRING);
							if (typeImport == 1) {
								ett.setCoNhaTieuKhongHVS(cell
										.getStringCellValue().trim().length());
							} else {
								if (cell.getStringCellValue().trim().length() > 0) {
						...
						case 8:
							...
							if (typeImport == 1) {
								...
								if (cell.getStringCellValue().length() > 0
										&& Validator.isNumber(cell
												.getStringCellValue())) {
									ett.setSoNguoi(Integer.valueOf(cell
						...
						case 16:
							if (typeImport == 1) {
								...
								if (Validator.isNotNull(cell
										.getStringCellValue().trim())) {
									try {
										ett.setDateToGet(sdfM.parse(cell
												.getStringCellValue().trim()));
										ett.setDateToGetStr(cell
												.getStringCellValue().trim());
									} catch (Exception e) {
										// TODO: handle exception
									}
			...
			if (erroType != 1) {
				for (int i = 0; i < listHuyen.size(); i++) {
					...
					if (i > 1) {
						try {
							// dataCheckOrg =
							// DataItemBusiness.getDATAITEMByTen(string,maTinh);
			...
			if (erroType == 0) {
				...
				if (results.size() > 6) {
					...
					for (int i = 0; i < 6; i++) {
						if (Validator.isNumber(results.get(i).getStt())
							...
							// dataItem =
							// DataItemBusiness.getDATAITEMByTen(results.get(i).getMaHuyen(),maTinh);
							dataItem = DataItemBusiness.getDATAITEMByTen(
									results.get(i).getMaHuyen(), maTinh, 2);
							...
							// dataItem2 =
							// DataItemBusiness.getDATAITEMByTen(results.get(i).getMaXa(),dataItem.getNode2());
							dataItem2 = DataItemBusiness.getDATAITEMByTen(
									results.get(i).getMaXa(),
									dataItem.getNode2(), 3);
							...
							_log.info("vo thread getMaXa"
									+ dataItem2.getNode3() + "");
							_log.info("vo thread getThonXom"
									+ results.get(i).getThonXom() + "");
							_log.info("vo thread getTenChuHo"
									+ results.get(i).getTenChuHo() + "");
							if (VeSinhGiaDinhLocalServiceUtil
									.findByT_CHK_Import(maTinh,
											dataItem.getNode2(),
											dataItem2.getNode3(),
											results.get(i).getThonXom(),
											results.get(i).getTenChuHo(),
				...
				if (isChk > 2) {
					...
					MessageBusUtil
							.sendMessage(
									"destination/sol/ns/daunoinuoc/HoGiaDinhImportThread",
									message);
				}
