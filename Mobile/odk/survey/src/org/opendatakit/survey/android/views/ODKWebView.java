...
public class ODKWebView extends WebView implements ServiceConnection {
	...
	public void onServiceConnected(ComponentName name, IBinder service) {
		dbShim = new ODKDbShimJavascriptCallback(ODKWebView.this, activity,
				(DbShimBinder) service);
	...
	public void onServiceDisconnected(ComponentName name) {
		log.w(t,
				"ODKWebView ServiceConnection.onServiceDisconnected() - disconnected from DbShimService!");
		dbShim = null;
	...
	protected Parcelable onSaveInstanceState() {
		...
		String[] waitQueue = new String[javascriptRequestsWaitingForPageLoad
				.size()];
		int i = 0;
		for (String s : javascriptRequestsWaitingForPageLoad) {
			waitQueue[i++] = s;
		}
		savedState.putStringArray(JAVASCRIPT_REQUESTS_WAITING_FOR_PAGE_LOAD,
				waitQueue);
		return savedState;
	}
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		if (savedState.containsKey(JAVASCRIPT_REQUESTS_WAITING_FOR_PAGE_LOAD)) {
			String[] waitQueue = savedState
					.getStringArray(JAVASCRIPT_REQUESTS_WAITING_FOR_PAGE_LOAD);
	 ...
	 * @param jsonObject
	 */
	public void doActionResult(String pageWaitingForData,
			String pathWaitingForData, String actionWaitingForData,
			String jsonObject) {
		// NOTE: this is asynchronous
		log.i(t, "doActionResult(" + pageWaitingForData + ", "
				+ pathWaitingForData + "," + actionWaitingForData + ",...)");
		loadJavascriptUrl("javascript:window.landing.opendatakitCallback('"
				+ pageWaitingForData + "','" + pathWaitingForData + "','"
				+ actionWaitingForData + "', '" + jsonObject + "' )");
		requestFocus();
	}
	...
	public void gotoUrlHash(String hash) {
		...
		loadJavascriptUrl("javascript:window.landing.opendatakitChangeUrlHash('"
				+ hash + "')");
	}
	...
	public synchronized void loadPage() {
		...
		String baseUrl = activity
				.getUrlBaseLocation(isLoadPageFrameworkFinished
						&& loadPageUrl != null);
		...
		if (baseUrl != null) {
			...
			log.w(t,
					"loadPage: framework did not load -- cannot load anything!");
		}
	}
	...
	public synchronized void clearPage() {
		...
		if (baseUrl != null) {
			...
			log.w(t,
					"clearPage: framework did not load -- cannot load anything!");
		}
	}
	...
	synchronized void frameworkHasLoaded() {
		...
		if (!isLoadPageFinished && !isJavascriptFlushActive) {
			log.i(t,
					"loadPageFinished: BEGINNING FLUSH refId: "
							+ activity.getRefId());
			...
			log.i(t, "loadPageFinished: IGNORING completion event refId: "
					+ activity.getRefId());
		}
	}
	...
	/**
	 * Tell the enclosing activity that we should restore this WebView to
	 * visible and make any custom view gone.
	 *
	...
	/**
	 * Tell the enclosing activity that we should make the indicated view
	 * visible and this one gone.
	 *
	 * NOTE: Only Invoked by ODKWebChromeClient.
	 *
	 * @param view
