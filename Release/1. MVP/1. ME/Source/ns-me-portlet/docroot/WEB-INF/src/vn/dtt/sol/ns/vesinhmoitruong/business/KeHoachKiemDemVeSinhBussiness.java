...
public class KeHoachKiemDemVeSinhBussiness {
	// fill data dau noi to list / reporting
		public static int countKeHoachKiemDemVeSInhByTypeUser(
				UserMapping userMapping, String maTinh, String maHuyen,String kiemDemVienId,String trangThai, int start, int end) {
			int results = 0;
			try {
				...
				if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TU) {
					if(Validator.isNotNull(maTinh)
						...
						results = KeHoachKiemDemVeSinhLocalServiceUtil.countKeHoachKiemDemVeSinhSearch(maTinh, maHuyen, kiemDemVienId, trangThai);
					}else{
					...
					results = KeHoachKiemDemVeSinhLocalServiceUtil.countKeHoachKiemDemVeSinhSearch(userMapping.getOrgCode1(),maHuyen, kiemDemVienId, trangThai);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_HUYEN) {
					results = KeHoachKiemDemVeSinhLocalServiceUtil.countKeHoachKiemDemVeSinhSearch(userMapping.getOrgCode1(),userMapping.getOrgCode2(), kiemDemVienId, trangThai);
				}
...
//				_log.info("maTinh"+maTinh+"maHuyen"+maHuyen+"maXa"+maXa+"tramCapNuocId"+tramCapNuocId+"trangThai"+trangThai+"namBaoCao"+namBaoCao);
				if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TU) {
					if(Validator.isNotNull(maTinh)
							||Validator.isNotNull(kiemDemVienId)
							||Validator.isNotNull(trangThai)){
						resultBase = KeHoachKiemDemVeSinhLocalServiceUtil.getKeHoachKiemDemVeSinhSearch(maTinh, maHuyen, kiemDemVienId, trangThai, start, end);
					}else{
						resultBase = KeHoachKiemDemVeSinhLocalServiceUtil.getKeHoachKiemDemVeSinhSearch("", "", "", "", start, end);
					}
					
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TINH) {
					resultBase = KeHoachKiemDemVeSinhLocalServiceUtil.getKeHoachKiemDemVeSinhSearch(userMapping.getOrgCode1(),maHuyen, kiemDemVienId, trangThai,start, end);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_HUYEN) {
					resultBase = KeHoachKiemDemVeSinhLocalServiceUtil.getKeHoachKiemDemVeSinhSearch(userMapping.getOrgCode1(), userMapping.getOrgCode2(), kiemDemVienId, trangThai,start, end);
				}
				int trangThaiChk = 0;
