...
public class DauNoiNuocBusiness {
		...
		for (DauNoiNuoc dauNoiNuoc : resultBase) {
			...
			results.add(new DauNoiNuocReportModel(dauNoiNuoc.getId(),
					...
					NuocSachUtils.getNameXa(dauNoiNuoc.getMaXa()), dauNoiNuoc
							...
							.getTenChuHo(), "tinh-trang-dau-noi-"
							+ dauNoiNuoc.getTrangThai(), Validator
	...
	public static List<ThongTinKiemDemNuocModel> getListThongTinKiemDemNuocModelByDauNoiId(
		...
		try {
			...
			for (THONGTINKIEMDEMNUOC thongtinkiemdemnuoc : resultBase) {
				...
				results.add(new ThongTinKiemDemNuocModel(thongtinkiemdemnuoc
						.getId(),
						Validator.isNotNull(KiemDemVienBussiness
								.findByTaiKhoan(thongtinkiemdemnuoc
										.getTaiKhoan())) ? KiemDemVienBussiness
								.findByTaiKhoan(
										thongtinkiemdemnuoc.getTaiKhoan())
								.getHoVaTen() : StringPool.BLANK, keHoachStr,
						thongtinkiemdemnuoc.getHoTenNguoiTraLoi(),
						"danh-gia-kiem-dem-"
								+ thongtinkiemdemnuoc.getDanhGiaKiemDem(),
						Validator.isNotNull(thongtinkiemdemnuoc
								.getNgayKiemDem()) ? sdf
								.format(thongtinkiemdemnuoc.getNgayKiemDem())
								: StringPool.BLANK,
						"kiem-dem-nuoc-da-ket-thuc-dieu-tra-"
								+ thongtinkiemdemnuoc.getDaKetThucDieuTra(), ""));
			}
	...
	public static List<DauNoiNuoc> getDauNoiGroupByMaTinhTW(String namBaoCao) {
		...
		try {
			lst = DauNoiNuocLocalServiceUtil
					.getDauNoiGroupByMaTinhTW(namBaoCao);
		} catch (Exception e) {
			_log.error(e);
		}
	...
	public static List<DauNoiNuoc> getDauNoiGroupByTramCapIDTW(
		...
		try {
			lst = DauNoiNuocLocalServiceUtil.getDauNoiGroupByTramCapIDTW(
					namBaoCao, trangThai);
		} catch (Exception e) {
			_log.error(e);
		}
