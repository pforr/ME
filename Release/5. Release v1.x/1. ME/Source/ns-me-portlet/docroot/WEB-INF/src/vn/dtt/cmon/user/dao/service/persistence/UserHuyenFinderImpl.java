...
public class UserHuyenFinderImpl extends BasePersistenceImpl<UserHuyen>
	...
	private List<UserMapping> finderU_IN_T_C(String maTinh, String maHuyen,
		...
		try {
			...
			if ((Validator.isNull(maHuyen) || maHuyen.contentEquals("0"))) {
				sql = StringUtil.replace(sql, "AND me_diabanqly_huyen.maHuyen = ?",
						StringPool.BLANK);
			}
			SQLQuery q = session.createSQLQuery(sql);
	...
	private List<UserMapping> finderGiamSatVien(String maTinh, String maHuyen,
			int start, int end) throws PortalException, SystemException {
