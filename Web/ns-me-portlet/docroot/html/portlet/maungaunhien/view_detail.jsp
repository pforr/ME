...
<%
	...
	/* rasoat thi phai sua doan cau lenh nay 
	KeHoachVeSinhLocalServiceUtil.finderHGD(tinh, huyen, xa, StringPool.BLANK, StringPool.BLANK, Integer.toString(nam), "rasoat", StringPool.BLANK, StringPool.BLANK, StringPool.BLANK, searchContainer.getStart(), searchContainer.getEnd())
	VeSinhGiaDinhBusiness.getVeSinhGiaDinhMauNgauNhienOnlySVRaSoat(tinh, huyen, xa, StringPool.BLANK, StringPool.BLANK, Integer.toString(nam), "rasoat", searchContainer.getStart(), searchContainer.getEnd())
	VeSinhGiaDinhBusiness.countVeSinhGiaDinhMauNgauNhienOnlySVRaSoat(tinh, huyen, xa, StringPool.BLANK, StringPool.BLANK, Integer.toString(nam), "rasoat")
	*/
...
<aui:form action="<%= searchURL %>" method="get" name="fm3">
	...
	<aui:fieldset>
		<liferay-ui:search-container delta="<%= DEFAULT_DELTA %>" iteratorURL="<%= searchURL %>" emptyResultsMessage="no-result-were-found" >
			...
			<liferay-ui:search-container-results results='<%= KeHoachVeSinhLocalServiceUtil.finderHGD(tinh, huyen, xa, StringPool.BLANK, StringPool.BLANK, Integer.toString(nam), "rasoat", StringPool.BLANK, StringPool.BLANK, StringPool.BLANK, searchContainer.getStart(), searchContainer.getEnd()) %>'
								 total='<%= KeHoachVeSinhLocalServiceUtil.countHGD(tinh, huyen, xa, StringPool.BLANK, StringPool.BLANK, Integer.toString(nam), "rasoat", StringPool.BLANK, StringPool.BLANK, StringPool.BLANK) %>'>
			...
			>
				...
				<liferay-ui:search-container-column-text
					name="trang-thai"
					value="<%= LanguageUtil.get(pageContext, \"trang-thai-ho-gia-dinh-\"+vsgd.getTrangThai())%>"
				/>
