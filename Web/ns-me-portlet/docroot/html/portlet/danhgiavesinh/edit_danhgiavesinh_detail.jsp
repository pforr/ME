...
// 							+ VeSinhGiaDinhLocalServiceUtil.countVeSinhGiaDinhSearch("", "", danhGiaVeSinhXaNew.getMaXa(), "", "3", "","");
	...
	countHGDCoNhaTieu = DanhGiaVeSinhXaNewFakeLocalServiceUtil.countHoGiaDinhCoNhaTieu(danhGiaVeSinhXaNew.getMaXa(), 2015);
	countHGDCoNhaTieuHVS = DanhGiaVeSinhXaNewFakeLocalServiceUtil.countHoGiaDinhCoNhaTieuHopVeSinh(danhGiaVeSinhXaNew.getMaXa(), 2015);
	
	int soHGDCoNhaVS = 0;
	int soHGDCoNhaTieuHVS = 0;
	
	if (danhGiaVeSinhXaNew.getSoHGDCoNhaTieu() > 0) {
		soHGDCoNhaVS = danhGiaVeSinhXaNew.getSoHGDCoNhaTieu();
	} else {
		soHGDCoNhaVS = countHGDCoNhaTieu;
	}

	if (danhGiaVeSinhXaNew.getSoHGDCoNhaTieuHVS() > 0) {
		soHGDCoNhaTieuHVS = danhGiaVeSinhXaNew.getSoHGDCoNhaTieuHVS();
	} else {
		soHGDCoNhaTieuHVS = countHGDCoNhaTieuHVS;
	}
	
	int soNguoiTrongXa = DanhGiaVeSinhXaNewFakeLocalServiceUtil.countSoNguoiTrongXa(danhGiaVeSinhXaNew.getMaXa());
...
<aui:form action="<%= editDanhGiaVesinhURL %>" method="post" name="fm">
	...
	<aui:input name="<%=DanhGiaVeSinhXaTerm.MA_XA %>" type="hidden" value="<%= danhGiaVeSinhXaNew.getMaXa() %>" />
	...
	<aui:input name="soDan" type="hidden" value="<%= Integer.toString(soNguoiTrongXa) %>" />
	...
	<aui:row cssClass="mHeigh">
		<aui:col width="40">
			...
			<aui:input cssClass="input-col2" name="<%=DanhGiaVeSinhXaTerm.SO_HGD_CO_NHA_TIEU %>" label="" value="<%=soHGDCoNhaVS %>" type="text" onChange='<%= renderResponse.getNamespace() + "updateValue();" %>' ></aui:input>
			</aui:field-wrapper>
			<aui:field-wrapper label="<%= LanguageUtil.get(pageContext, \"so-hgd-co-nha-tieu-hvs\") +dotLabel %>" >
			<aui:input cssClass="input-col2" name="<%=DanhGiaVeSinhXaTerm.SO_HGD_CO_NHA_TIEU_HVS %>" label="" type="text" value="<%= soHGDCoNhaTieuHVS %>" onChange='<%= renderResponse.getNamespace() + "updateValue();" %>' ></aui:input>
			</aui:field-wrapper>
...
function <portlet:namespace />updateValue() {
	...
	var row2 = A.one("#<portlet:namespace/>soHGDCoNhaTieu");
