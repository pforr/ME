...
//			_log.info("maTinh"+maTinh+"maHuyen"+maHuyen+"maXa"+maXa+"tramCapNuocId"+tramCapNuocId+"trangThai"+trangThai+"namBaoCao"+namBaoCao);
			...
			for (KeHoachKiemDemNuoc keHoachKiemDemNuoc : resultBase) {
				...
				try {
					results.add(new KeHoachKiemDemNuocReportModel(keHoachKiemDemNuoc.getId(), 
							...
							KiemDemVienBussiness.getKiemDemVien(keHoachKiemDemNuoc.getKiemDemVienId()).getHoVaTen(), 
	...
	// fill data dau noi to list / reporting
		public static int countKeHoachKiemDemNuocByTypeUser(
				UserMapping userMapping, String maTinh,String kiemDemVienId,String tramCapNuocId,String trangThai,String nam, int start, int end) {
			int results = 0;
			try {
				...
				if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TU) {
					if(!maTinh.isEmpty()
							...
							||!nam.isEmpty()
							||!trangThai.isEmpty()){
						results = KeHoachKiemDemNuocLocalServiceUtil.countKeHoachKiemDemNuocSearchNam(maTinh,"", kiemDemVienId, tramCapNuocId, trangThai,nam);
					}else{
						results = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocsCount();
					}
					
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TINH) {
					results = KeHoachKiemDemNuocLocalServiceUtil.countKeHoachKiemDemNuocSearchNam(userMapping.getOrgCode1(),"", kiemDemVienId, tramCapNuocId, trangThai,nam);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_HUYEN) {
					results = KeHoachKiemDemNuocLocalServiceUtil.countKeHoachKiemDemNuocSearchNam(userMapping.getOrgCode1(),userMapping.getOrgCode2(), kiemDemVienId, tramCapNuocId, trangThai,nam);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TRAM) {
					TramCapNuoc tramCapUser = null;
					tramCapUser = TramCapBusiness.getTramCap(Long.valueOf(userMapping.getOrgCode2()));
					results = KeHoachKiemDemNuocLocalServiceUtil.countKeHoachKiemDemNuocSearchNam(tramCapUser.getMaTinh(), tramCapUser.getMaHuyen(), kiemDemVienId, tramCapUser.getId()+"", trangThai,nam);
				}
...
//				_log.info("maTinh"+maTinh+"maHuyen"+maHuyen+"maXa"+maXa+"tramCapNuocId"+tramCapNuocId+"trangThai"+trangThai+"namBaoCao"+namBaoCao);
				if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TU) {
					if(!maTinh.isEmpty()
							...
							||!nam.isEmpty()
							||!trangThai.isEmpty()){
						resultBase = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocSearchNam(maTinh,"", kiemDemVienId, tramCapNuocId, trangThai, nam,start, end);
					}else{
						resultBase = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocSearchNam("","", "", "", "","",start, end);
					}
					
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TINH) {
					resultBase = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocSearchNam(userMapping.getOrgCode1(),"", kiemDemVienId, tramCapNuocId, trangThai, nam,start, end);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_HUYEN) {
					resultBase = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocSearchNam(userMapping.getOrgCode1(), userMapping.getOrgCode2(), kiemDemVienId, tramCapNuocId, trangThai, nam,start, end);
				}else if (userMapping.getUserType() == KeHoachKiemDemNuocConstantString.UserMapping.CAP_TRAM) {
					TramCapNuoc tramCapUser = null;
					tramCapUser = TramCapBusiness.getTramCap(Long.valueOf(userMapping.getOrgCode2()));
					resultBase = KeHoachKiemDemNuocLocalServiceUtil.getKeHoachKiemDemNuocSearchNam(tramCapUser.getMaTinh(), tramCapUser.getMaHuyen(), kiemDemVienId, tramCapUser.getId()+"", trangThai, nam,start, end);
				}
				int trangThaiChk = 0;
				for (KeHoachKiemDemNuoc keHoachKiemDemNuoc : resultBase) {
					if(Validator.isNull(keHoachKiemDemNuoc.getNgayXuatBan()) && Validator.isNull(keHoachKiemDemNuoc.getNgayDong())){
...
//					tongDauNoi = DauNoiNuocBusiness.countDauNoiByTypeUser(initUserType, "", dataItem.getNode1(), dataItem.getNode2(), dataItem.getNode3(), "", "", "", start, end);
		...
		public static List<ThongTinKiemDemNuocModel> getListThongTinKiemDemNuocModelViewDetail(
				int keHoachId,String trangThai,String maTinh,String maHuyen,String maXa,String nguonDL, String danhGiaKiemDem, int start, int end) {
			List<ThongTinKiemDemNuocModel> results = new ArrayList<ThongTinKiemDemNuocModel>();
			List<THONGTINKIEMDEMNUOC> resultBase = new ArrayList<THONGTINKIEMDEMNUOC>();
				try {
					
					resultBase = THONGTINKIEMDEMNUOCLocalServiceUtil.getThongTinKeHoachKiemDemNuocSearch(keHoachId+"", trangThai, maTinh, maHuyen, maXa,nguonDL,danhGiaKiemDem, start, end);
		...
		public static int countListThongTinKiemDemNuocModelViewDetail(
				int keHoachId,String trangThai,String maTinh,String maHuyen,String maXa,String nguonDL, String danhGiaKiemDem) {
			int result = 0;
				try {
					
					result = THONGTINKIEMDEMNUOCLocalServiceUtil.getThongTinKeHoachKiemDemNuocSearch(keHoachId+"", trangThai, maTinh, maHuyen, maXa,nguonDL, danhGiaKiemDem, QueryUtil.ALL_POS, QueryUtil.ALL_POS).size();
