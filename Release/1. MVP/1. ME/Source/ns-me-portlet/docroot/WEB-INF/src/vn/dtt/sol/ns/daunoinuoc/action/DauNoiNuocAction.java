...
public class DauNoiNuocAction extends MVCPortlet {
	...
	public void doView(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException {
		...
		String tramCapNuocId = ParamUtil.getString(renderRequest, DauNoiNuocTerms.TRAM_CAP_NUOC_ID);
		String trangThai = ParamUtil.getString(renderRequest, DauNoiNuocTerms.TRANG_THAI);
		String namBaoCao = ParamUtil.getString(renderRequest, DauNoiNuocTerms.NAM_BAO_CAO);
		...
		try {
			 ...
			 long dauNoiId = ParamUtil.getLong(renderRequest, DauNoiNuocTerms.ID);
			 if(dauNoiId>0){
...
//				tramCap = TramCapBusiness.getTramCap(Long.valueOf(tramCapNuocId));
//				if(Validator.isNotNull(tramCap)){
//					listTinh = new ArrayList<DATAITEM>();
//					listTinh.add(NuocSachUtils.getDATAITEMTinh(tramCap.getMaTinh()));
//					listHuyen = DataItemBusiness.getInLevel2(WebKeys.DATAGROUPID_DONVIHANHCHINH, tramCap.getMaTinh());
//				}
//			}
		...
		renderRequest.setAttribute(DauNoiNuocTerms.TRAM_CAP_NUOC_ID, tramCapNuocId);
...
//			}
	...
	public void addDauNoiNuoc(ActionRequest actionRequest, ActionResponse actionResponse) throws IOException {
		...
		try {
			...
			if (add.equals(Constants.ADD)) {
				...
				dauNoiNuocEtt.setIdNguoiTao(Integer.valueOf(String.valueOf(userId)));
			...
			dauNoiNuocEtt.setNgayDauNoiHopDong(dauNoiNuocTerm.getNgayDauNoiHopDong());
			...
			resultChk = DauNoiNuocLocalServiceUtil.getChkAddDauNoi(dauNoiNuocEtt.getTramCapNuocId(), dauNoiNuocEtt.getMaTinh(), dauNoiNuocEtt.getMaHuyen(), dauNoiNuocEtt.getMaXa(), dauNoiNuocEtt.getThonXom(), dauNoiNuocEtt.getTenChuHo());
	...
	public void addDanhGiaLai(ActionRequest actionRequest, ActionResponse actionResponse) {
		DauNoiNuocTerms dauNoiNuocTerm = new DauNoiNuocTerms(actionRequest);
	...
	public void deleteDauNoiNuoc(ActionRequest actionRequest, ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
...
//						isNext = 0;
						...
						if(cell.getRowIndex()==6 && col == 1){
							...
							try {
								dataCheckOrg = DataItemBusiness.getDATAITEMByTen(tinhStr,"");
								importName = tinhStr;
...
//						_log.info("col"+col+"tye"+cell.getCellType()+"/"+cell.getRowIndex()+"/");
						...
						case 2:
							...
							ett.setTenHeThongCapNuoc(cell.getStringCellValue().trim());
							break;
						case 3:
							cell.setCellType(Cell.CELL_TYPE_STRING);
						...
						case 6:
							cell.setCellType(Cell.CELL_TYPE_STRING);
							ett.setTenChuHoCoQuan(cell.getStringCellValue().trim());
							break;
						case 7:
							cell.setCellType(Cell.CELL_TYPE_STRING);
								ett.setSoNguoiTrongHo(cell.getStringCellValue().trim());
							break;
						case 8:
							cell.setCellType(Cell.CELL_TYPE_STRING);
							try {
								ett.setNgayDauNoiDate(sdf.parse(cell.getStringCellValue()));
...
//							}
			...
			listTram = new ArrayList<String>(new LinkedHashSet<String>(listTram));
...
//							dataCheckOrg = DataItemBusiness.getDATAITEMByTen(tinhStr,"");
//							dataCheckOrg = DataItemBusiness.getDATAITEMByTen(string,dataCheckOrg.getNode1());
							dataCheckOrg = DataItemBusiness.getDATAITEMByTen(tinhStr,"",1);
							dataCheckOrg = DataItemBusiness.getDATAITEMByTen(string,dataCheckOrg.getNode1(),2);
							importName = string;
...
//							dataCheckOrg = DataItemBusiness.getDATAITEMByTen(string,"");
				...
				for (int i=0; i< listTram.size(); i++) {
					...
					if(listTramCapNuoc.size()>0){
						for (TramCapNuoc tramCapNuoc : listTramCapNuoc) {
							_log.info("tramCapNuoc.getMaTinh()"+tramCapNuoc.getMaTinh()+"//"+maTinh);
							if(!tramCapNuoc.getMaTinh().equalsIgnoreCase(maTinh)){
								erroType = 4;
								importName = string;
								break;
							}
			...
			if(erroType == 0){
				...
				if(results.size() > 6){
					for (int i=0; i<6;i++) {
						_log.info(results.get(i).getHuyen()+"//"+results.get(i).getXa()+"Validator.isNumber(results.get(i).getStt())"+Validator.isNumber(results.get(i).getStt()));
						if( Validator.isNumber(results.get(i).getStt())
								&&results.get(i).getHuyen().length()>4){
							long idTram = 0;
							idTram = TramCapBusiness.getByTenTram(results.get(i).getTenHeThongCapNuoc()).size()>0?TramCapBusiness.getByTenTram(results.get(i).getTenHeThongCapNuoc()).get(0).getId():0;
...
//							dataItem = DataItemBusiness.getDATAITEMByTen(results.get(i).getTinh(),"");
							dataItem = DataItemBusiness.getDATAITEMByTen(results.get(i).getTinh(),"",1);
//							if(dataItem.getLevel()==1)results.get(i).setMaTinh(dataItem.getNode1());
//							dataItem2 = DataItemBusiness.getDATAITEMByTen(results.get(i).getHuyen(),dataItem.getNode1());
							dataItem2 = DataItemBusiness.getDATAITEMByTen(results.get(i).getHuyen(),dataItem.getNode1(),2);
//							if(dataItem.getLevel()==2)results.get(i).setMaHuyen(dataItem.getNode2());
//							dataItem3 = DataItemBusiness.getDATAITEMByTen(results.get(i).getXa(),dataItem2.getNode2());
							dataItem3 = DataItemBusiness.getDATAITEMByTen(results.get(i).getXa(),dataItem2.getNode2(),3);
//							if(dataItem.getLevel()==3)results.get(i).setMaXa(dataItem.getNode3());
							_log.info("vo thread getTenHeThongCapNuocId"+Integer.valueOf(idTram+""));
							_log.info("vo thread getMaTinh"+ dataItem.getNode1()+"");
//							_log.info("vo thread getMaHuyen"+ dataItem2.getNode2()+"");
//							_log.info("vo thread getMaXa"+ dataItem3.getNode3()+"");
							if(DauNoiNuocLocalServiceUtil.getChkImport(Integer.valueOf(idTram+""), 
									dataItem.getNode1(), 
									dataItem2.getNode2(), 
									dataItem3.getNode3(), 
				...
				if(isChk>3){
					...
					int namBaoCao = ParamUtil.getInteger(actionRequest, DauNoiNuocTerms.NAM_BAO_CAO);
					...
					MessageBusUtil.sendMessage("destination/sol/ns/daunoinuoc/DauNoiNuocImportThread", message);
				}
...
//			SessionMessages.add(actionRequest, PortalUtil.getPortletId(actionRequest) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_SUCCESS_MESSAGE);
			String successURL = ParamUtil.getString(actionRequest, "onpageSuccess");
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_TINH+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_TINH);
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_HUYEN+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_HUYEN);
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_XA+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_XA);
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.TRAM_CAP_NUOC_ID+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.TRAM_CAP_NUOC_ID);
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.TRANG_THAI+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.TRANG_THAI);
			successURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.NAM_BAO_CAO+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.NAM_BAO_CAO);
			if(id>0){
...
//			SessionMessages.add(actionRequest, PortalUtil.getPortletId(actionRequest) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_ERROR_MESSAGE);	
			String errorURL = ParamUtil.getString(actionRequest, "onpageError");
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_TINH+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_TINH);
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_HUYEN+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_HUYEN);
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.MA_XA+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.MA_XA);
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.TRAM_CAP_NUOC_ID+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.TRAM_CAP_NUOC_ID);
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.TRANG_THAI+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.TRANG_THAI);
			errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.NAM_BAO_CAO+"="+ParamUtil.getString(actionRequest, DauNoiNuocTerms.NAM_BAO_CAO);
			if(id>0){
				errorURL+="&"+actionResponse.getNamespace()+"duplicate=1";
				errorURL+="&"+actionResponse.getNamespace()+DauNoiNuocTerms.ID+"="+id;
			}
			if(messageKey.equalsIgnoreCase(MessageSuccess.ADD_DAU_NOI_DUPLICATE_MESSAGE)){
				SessionMessages.add(actionRequest, messageKey);
			}else{
				SessionErrors.add(actionRequest, messageKey);
			}
