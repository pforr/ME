...
<%
final Log log = LogFactoryUtil.getLog("html.portlet.kehoachkiemdemvesinh.view_detail_tab1_add_ho_gd.jsp");
...
log.info("Nam Bao Cao --html.portlet.kehoachkiemdemvesinh.view_detail_tab1_add_ho_gd.jsp--HGD-----"+namBaoCaovstx);
//namBaoCaovstx = "rasoat";/// phuc vu cho con rasoatlttai
log.info("Nam Bao Cao da set lai ---html.portlet.kehoachkiemdemvesinh.view_detail_tab1_add_ho_gd.jsp-------"+namBaoCaovstx);
if(Validator.isNotNull(maHuyen)){
	//listXa = NuocSachUtils.getDiaBanHuyen(user, maHuyen);
 	listXa = DataItemBusiness.getInLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH, keHoachKiemDemVeSinh.getMaTinh(), maHuyen); /// lttai
	log.info("--html.portlet.kehoachkiemdemvesinh.view_detail_tab1_add_ho_gd.jsp-listXa---"+listXa.size());
}
log.info("--html.portlet.kehoachkiemdemvesinh.view_detail_tab1_add_ho_gd.jsp--maHuyen--"+maHuyen+"====");
...
<%-- 		</aui:row> --%>
	...
	<aui:fieldset>
		<liferay-portlet:actionURL var="themDauNoiToKeHoachKieDemVeSinhURL" name="themDauNoiToKeHoachKieDemVesinhHoGDId" >
		...
		<liferay-portlet:actionURL var="themAllDauNoiToKeHoachKieDemVesinhURL" name="themAllDauNoiToKeHoachKieDemVesinh" >
			<liferay-portlet:param name="<%=KeHoachKiemDemVeSinhTerms.MA_TINH %>" value="<%=keHoachKiemDemVeSinh.getMaTinh() %>"></liferay-portlet:param>
			<liferay-portlet:param name="<%=KeHoachKiemDemVeSinhTerms.MA_HUYEN %>" value="<%=keHoachKiemDemVeSinh.getMaHuyen() %>"></liferay-portlet:param>
			<liferay-portlet:param name="idTrangThaiChk" value="<%=String.valueOf(idTrangThaiChk) %>"></liferay-portlet:param>
			<liferay-portlet:param name="trangThai" value="<%=trangThai %>"></liferay-portlet:param>
		...
		<liferay-portlet:renderURL varImpl="searchURL" >
			<portlet:param name="mvcPath" value="/html/portlet/kehoachkiemdemvesinh/view_detail_tab1_add_ho_gd.jsp" />
		...
		<aui:form action="<%= searchURL %>" method="post" name="fm">
			<aui:script>
				Liferay.provide(window,'<portlet:namespace />addDauNoi', function() {
					...
					if (confirm('<%= UnicodeLanguageUtil.get(pageContext, "Có thêm các hộ gia đình được chọn vào kế hoạch kiểm đếm vệ sinh? ") %>')) {
						...
						submitForm(document.<portlet:namespace />fm, "<%=themDauNoiToKeHoachKieDemVeSinhURL.toString()%>");
					}
				...
				Liferay.provide(window,'<portlet:namespace />addAllDauNoi', function() {
					if (confirm('<%= UnicodeLanguageUtil.get(pageContext, "Có thêm toàn bộ hộ gia đình vào kế hoạch kiểm đếm vệ sinh? ") %>')) {
						submitForm(document.<portlet:namespace />fm, "<%=themAllDauNoiToKeHoachKieDemVesinhURL.toString()%>");
					}
				},['liferay-util-list-fields']);
			</aui:script>
			<aui:input name="onpageError" type="hidden" value="<%= onpageError%>" />
			...
			<liferay-portlet:renderURL varImpl="iteratorURL">
				...
				<portlet:param name="idNguoiTao" value="<%=idNguoiTao %>" />
				<portlet:param name="advText" value="<%=advText %>" />
				<portlet:param name="namBaoCaovstx" value="<%= namBaoCaovstx %>"/>
				<portlet:param name="<%=KeHoachKiemDemVeSinhTerms.ID %>" value="<%=String.valueOf(keHoachKiemDemVeSinh.getId()) %>" />
			</liferay-portlet:renderURL>
			<liferay-ui:search-container iteratorURL="<%= iteratorURL %>" delta="50" rowChecker="<%= new RowChecker(renderResponse) %>" searchContainer="<%=new VeSinhMoiTruongSearch(renderRequest, 50, SearchContainer.DEFAULT_ORDER_BY_COL_PARAM, SearchContainer.DEFAULT_ORDER_BY_TYPE_PARAM, iteratorURL) %>" >
				<%@ include file="/html/portlet/kehoachkiemdemvesinh/include_kehoachkiemdemvesinh_detail_form.jspf"%>
				<liferay-ui:search-container-results>
					<%@ include file="/html/portlet/kehoachkiemdemvesinh/search_hogiadinh_kehoach_database_result.jspf"%>
				</liferay-ui:search-container-results>
