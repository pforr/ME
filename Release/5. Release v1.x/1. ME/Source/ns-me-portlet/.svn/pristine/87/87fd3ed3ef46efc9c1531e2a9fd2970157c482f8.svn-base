...
*/
	...
	private void updateStatusHoGiaDinh(ThongTinKiemDemHoGD thongtinkiemdemhogd,
		...
		// type  = 1 la xoa ke hoach, type = 2 la dong ke hoach
		// danhgia
		if(type == 1){ // xoa kehoach
			ettUpdate.setTrangThai(0); //vesinhgiadinh trang thai chuyen ve chua kiemdem
		} else if (type == 2){ // dong ke hoach
			if (thongtinkiemdemhogd.getDaKetThucDieuTra() == 0) { //  dangchokhaosat
				thongtinkiemdemhogd.setDaKetThucDieuTra(3); // gia dinh trong ke hoach thanh DAHUY
				ettUpdate.setTrangThai(0); //vesinhgiadinh trang thai chuyen ve chua kiemdem
			}
			if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 1) { // danh gia la hvs xay moi
				ettUpdate.setTrangThai(2); // vesinhgiadinh se chuyen thanh co nha tieu hvs xay moi
			} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 2) { // danh gia la hvs binh thuong
				ettUpdate.setTrangThai(3); // vesinhgiadinh se chuyen thanh co nha tieu hvs binh thuong
			} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 3) { // danh gia la nha tieu KO hvs
				ettUpdate.setTrangThai(4); // vesinhgiadinh se chuyen thanh nha tieu KO HVS
			} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 4) { // danh gia la ko co nha tieu
				ettUpdate.setTrangThai(5); // vesinhgiadinh se chuyen thanh khong co nha tieu
			}
		}
		/*
		if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 0) { // danhgia ko co ket qua
			ettUpdate.setTrangThai(0); // vesinhgiadinh se chuyen ve chua kiem dem
		} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 1) { // danh gia la hvs xay moi
			ettUpdate.setTrangThai(2); // vesinhgiadinh se chuyen thanh co nha tieu hvs xay moi
		} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 2) { // danh gia la hvs binh thuong
			ettUpdate.setTrangThai(3); // vesinhgiadinh se chuyen thanh co nha tieu hvs binh thuong
		} else if (thongtinkiemdemhogd.getDanhGiaKiemDem() == 3) { // danh gia la nha tieu KO hvs
	...
	private void updateStatusCongTrinh(
		...
		if(type == 1){ // xoa kehoach
			...
			if (thongtinkiemdemcongtrinh.getDanhGiaKiemDem() == 0) { // congtrinh dang la chokhaosat
				thongtinkiemdemcongtrinh.setDaKetThucDieuTra(3); // congtrinh trong ke hoach chuyen thanh dahuy
				ettUpdate.setTrangThai(0);// congtrinh goc chuyen thanh chua co ke hoach kiemdem
			} else if (thongtinkiemdemcongtrinh.getDanhGiaKiemDem() == 1) { // congtrinh danhgia hvs
				ettUpdate.setTrangThai(2);// congtrinh goc chuyen thanh hvs
			} else if (thongtinkiemdemcongtrinh.getDanhGiaKiemDem() == 2) { // congtrinh danhgia ko hvs
				ettUpdate.setTrangThai(3);// congtrinh goc chuyen thanh ko hvs
			}
		}
		/*
				...
				if (Validator.isNotNull(ettThongTin)
					...
					ettRevert = VeSinhGiaDinhLocalServiceUtil
							.fetchVeSinhGiaDinh(ettThongTin
									.getVeSinhGiaDinhId());
					//ettRevert.setTrangThai(8); cu la 8
					ettRevert.setTrangThai(0);
	...
	public void dongkeHoachKiemDem(ActionRequest actionRequest,
		...
		try {
			id = ParamUtil.getInteger(actionRequest,
					KeHoachKiemDemVeSinhTerms.ID);
			KeHoachKiemDemVeSinh keHoachKiemDemVeSinh = new KeHoachKiemDemVeSinhImpl();
			_log.info("---dongkeHoachKiemDem---KeHoachKiemDemVeSinhTerms.ID---" + id+"===");
			if (id > 0) {
				keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhLocalServiceUtil
						.fetchKeHoachKiemDemVeSinh(id);
				messageKey = MessageSuccess.DONG_KE_HOACH_KIEM_DEM_MESSAGE;
