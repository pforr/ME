		...
		<c:if test='<%= type.equals("regular") %>'>
			<c:if test="<%= PropsValues.SEARCH_CONTAINER_PAGE_DELTA_VALUES.length > 0 %>">
				<div class="lfr-pagination-config">
					<div class="lfr-pagination-page-selector" style="padding-right: 10px;">
						<c:choose>
			...
			<c:if test='<%= type.equals("regular") %>'>
				<li class="<%= (cur != pages) ? "" : "disabled" %> last">
					...
					<div class="lfr-pagination-delta-selector">
						<div style="margin-right: 5px; margin-top: 6px;">
							<%= LanguageUtil.format(pageContext, "sum-x-result", numberFormat.format(total)) %>
						</div>
