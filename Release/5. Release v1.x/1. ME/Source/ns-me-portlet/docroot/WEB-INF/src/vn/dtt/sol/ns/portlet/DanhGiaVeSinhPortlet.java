...
public class DanhGiaVeSinhPortlet extends MVCPortlet {
	...
	public void doView(RenderRequest renderRequest,
		...
		String maTinh = ParamUtil.getString(renderRequest,
				DanhGiaVeSinhXaTerm.MA_TINH);
		String maHuyen = ParamUtil.getString(renderRequest,
				DanhGiaVeSinhXaTerm.MA_HUYEN);
		String maXa = ParamUtil.getString(renderRequest,
				DanhGiaVeSinhXaTerm.MA_XA);
		String namBaoCao = ParamUtil.getString(renderRequest,
				DanhGiaVeSinhXaTerm.NAM);
		...
		try {
			...
			int dauNoiId = ParamUtil.getInteger(renderRequest,
					DanhGiaVeSinhXaTerm.ID);
			if (dauNoiId > 0) {
				danhGiaVeSinhXaNew = DanhGiaVeSinhXaBussiness
						.fetchDanhGiaVeSinhXaById(dauNoiId);
				maTinh = danhGiaVeSinhXaNew.getMaTinh();
				maHuyen = danhGiaVeSinhXaNew.getMaHuyen();
				renderRequest.setAttribute("danhGiaVeSinhXaNew",
						danhGiaVeSinhXaNew);
			}
			if (loginUserMapping.getUserType() == DauNoiNuocConstantString.UserMapping.CAP_TU) {
				listTinh = DATAITEMLocalServiceUtil
						.getByDataGroupLevel1(WebKeys.DATAGROUPID_DONVIHANHCHINH);
				if (Validator.isNotNull(maTinh)) {
					listHuyen = DataItemBusiness.getInLevel2(
							WebKeys.DATAGROUPID_DONVIHANHCHINH, maTinh);
				}
				if (Validator.isNotNull(maHuyen)) {
					listXa = DataItemBusiness
							.getInLevel3(WebKeys.DATAGROUPID_DONVIHANHCHINH,
									maTinh, maHuyen);
				}
			} else if (loginUserMapping.getUserType() == DauNoiNuocConstantString.UserMapping.CAP_TINH) {
				maTinh = loginUserMapping.getOrgCode1();
				listTinh.add(NuocSachUtils.getDATAITEMTinh(loginUserMapping
						.getOrgCode1()));
				if (Validator.isNotNull(loginUserMapping.getOrgCode1())) {
					listHuyen = DataItemBusiness.getInLevel2(
							WebKeys.DATAGROUPID_DONVIHANHCHINH,
							loginUserMapping.getOrgCode1());
				}
				if (Validator.isNotNull(maHuyen)) {
					listXa = DataItemBusiness.getInLevel3(
							WebKeys.DATAGROUPID_DONVIHANHCHINH,
							loginUserMapping.getOrgCode1(), maHuyen);
				}
				...
				listTinh.add(NuocSachUtils.getDATAITEMTinh(loginUserMapping
						.getOrgCode1()));
				listHuyen.add(NuocSachUtils.getDATAITEMHuyen(
						loginUserMapping.getOrgCode1(),
				...
				tramCapUser = TramCapBusiness.getTramCap(Long
						.valueOf(loginUserMapping.getOrgCode2()));
				listTinh.add(NuocSachUtils.getDATAITEMTinh(tramCapUser
						.getMaTinh()));
				if (Validator.isNotNull(tramCapUser.getMaHuyen())) {
					listHuyen.add(NuocSachUtils.getDATAITEMHuyen(
							tramCapUser.getMaTinh(), tramCapUser.getMaHuyen()));
				} else {
					listHuyen = DataItemBusiness.getInLevel2(
							WebKeys.DATAGROUPID_DONVIHANHCHINH,
							tramCapUser.getMaTinh());
				}
				listXa = DataItemBusiness.getInLevel3(
						WebKeys.DATAGROUPID_DONVIHANHCHINH,
						tramCapUser.getMaTinh(), tramCapUser.getMaHuyen());
	...
	public void render(RenderRequest renderRequest,
		...
		try {
			...
			if (dauNoiId > 0) {
				danhGiaVeSinhXaNew = DanhGiaVeSinhXaBussiness
						.fetchDanhGiaVeSinhXaById(dauNoiId);
				_log.info("tinh: " + danhGiaVeSinhXaNew.getMaTinh() + "huyen: "
						+ danhGiaVeSinhXaNew.getMaHuyen() + "xa: "
						+ danhGiaVeSinhXaNew.getMaXa() + "nam: "
						+ danhGiaVeSinhXaNew.getNam());
				MAUKIEMDEM mkd = MAUKIEMDEMLocalServiceUtil
						.getMauKiemDemByTHXN(danhGiaVeSinhXaNew.getMaTinh(),
								danhGiaVeSinhXaNew.getMaXa(),
				...
				renderRequest.setAttribute("tongSoHGDTrongMau",
						tongSoHGDTrongMau);
				renderRequest.setAttribute("danhGiaVeSinhXaNew",
						danhGiaVeSinhXaNew);
			}
	...
	public void editDanhGiaVesinhDetail(ActionRequest actionRequest,
		...
		try {
			ett = DanhGiaVeSinhXaNewLocalServiceUtil
					.fetchDanhGiaVeSinhXaNew(ettTerms.getId());
			ett.setSoNhaTieuHVSXayMoi(ettTerms.getSoNhaTieuHVSXayMoi());
			ett.setSoHGDCoNhaTieu(ettTerms.getSoHGDCoNhaTieu());
			int soDan = 0;
			try {
				soDan = DanhGiaVeSinhXaNewFakeLocalServiceUtil
						.countSoNguoiTrongXa(ettTerms.getMaXa());
				System.out.print("So danXXXXXXXXXXXXx: " + soDan);
	...
	private void updateVeSinhHGDFromDanhGiaVSTX(String maXa, int nam)
			throws SystemException {
		List<VeSinhGiaDinh> listToUpdate = null;
		listToUpdate = VeSinhGiaDinhLocalServiceUtil.getVeSinhGiaDinhSearch("",
				"", maXa, "", "2", "0", "", QueryUtil.ALL_POS,
				QueryUtil.ALL_POS);
	...
	private void updateVeSinhCTFromDanhGiaVSTX(String maXa, int nam)
			throws SystemException {
		List<VeSinhCongTrinh> listToUpdate = null;
		listToUpdate = VeSinhCongTrinhLocalServiceUtil
				.getVeSinhCongTrinhSearch("", "", maXa, "", "1", "2", "0",
		...
		listToUpdate = VeSinhCongTrinhLocalServiceUtil
				.getVeSinhCongTrinhSearch("", "", maXa, "", "2", "2", "0",
						QueryUtil.ALL_POS, QueryUtil.ALL_POS);
		for (VeSinhCongTrinh veSinhCongTrinh : listToUpdate) {
			veSinhCongTrinh.setNgayDanhGia(new Date());
			veSinhCongTrinh.setNamBaoCao(nam);
	...
	public void editDanhGiaVesinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		String messageKey = MessageErrors.EDIT_DANH_GIA_VE_SINH_MESSAGE;
		long idReturn = 0;
		...
		try {
			...
			int danhGia = danhGiaVSTX(ettTerms.getTiLeHGDCoNhaTieu(),
					ettTerms.getTiLeHGDCoNhaTieuHVS(),
					ettTerms.getTiLeTruongHocHVS(),
					ettTerms.getTiLeTramYTeHVS());

			ett.setKetLuanVSTX(danhGia);
	...
	private int danhGiaVSTX(int tiLeHGDCoNhaTieu, int tiLeHGDCoNhaTieuHVS,
			int tiLeTruongHocHVS, int tiLeTramYTeHVS) {
		int danhGia = 1;

		if (tiLeHGDCoNhaTieu < 100) {
			danhGia = 2;
		}

		if (tiLeHGDCoNhaTieuHVS < 70) {
			danhGia = 2;
		}
	...
	public void deleteDanhGiaVeSinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		...
		try {
			...
			listToDel = DanhGiaVeSinhXaNewLocalServiceUtil
					.getListInsertVeSinhXa(ettTerms.getMaTinh(),
							ettTerms.getMaHuyen(), ettTerms.getNam());
			_log.info(ettTerms.getMaTinh() + "/" + ettTerms.getMaHuyen() + "/"
					+ ettTerms.getNam() + "/" + listToDel.size());
			for (DanhGiaVeSinhXaNew danhGiaVeSinhXaNew : listToDel) {
				DanhGiaVeSinhXaNewLocalServiceUtil
						.deleteDanhGiaVeSinhXaNew(danhGiaVeSinhXaNew);
			}
