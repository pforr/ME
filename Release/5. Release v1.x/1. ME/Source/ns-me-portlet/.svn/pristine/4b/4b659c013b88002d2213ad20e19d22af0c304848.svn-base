...
<%
	...
	requiredLabel ="(*)";
	String dotLabel=":";
	long selTramCap = ParamUtil.getLong(request, "selCapTinhId");
%>
<aui:select cssClass="input1001" required="true" name="<%=DauNoiNuocTerms.TRAM_CAP_NUOC_ID %>" id="<%=DauNoiNuocTerms.TRAM_CAP_NUOC_ID %>" label="<%= LanguageUtil.get(pageContext, \"tram-cap-nuoc\") + dotLabel %>" showEmptyOption="<%= false %>">
	...
	%>
	<aui:option selected="<%= selTramCap == tramCapNuoc.getId() %>" value="<%= tramCapNuoc.getId() %>"><%= tramCapNuoc.getTenTram() %></aui:option>
	<%
		}
	%>
</aui:select>
