...
public class MauNgauNhienBusiness {
	...
	public static int countLayMau(int nam, String maTinh, String maHuyen, String maXa,
			boolean isRaSoat) {
		int count = 0;

		try {
			if (isRaSoat) {
				count = MAUKIEMDEMLocalServiceUtil.countByRaSoat(nam, maTinh,
						maHuyen, maXa);
			} else {
				count = MAUKIEMDEMLocalServiceUtil.countByKiemDem(nam, maTinh,
						maHuyen, maXa);
			}
		} catch (Exception e) {
			_log.error(e);
	...
	public static List<VeSinhGiaDinh> findLayMau(int nam, String maTinh,
			String maHuyen, String maXa, int start, int end, boolean isRaSoat) {
		List<VeSinhGiaDinh> ls = new ArrayList<VeSinhGiaDinh>();

		try {
			if (isRaSoat) {
				ls = MAUKIEMDEMLocalServiceUtil.findByRaSoat(nam, maTinh,
						maHuyen, maXa, start, end);
			} else {
				ls = MAUKIEMDEMLocalServiceUtil.findByKiemDem(nam, maTinh,
						maHuyen, maXa, start, end);
			}
	...
	public static int countVSGD(String maTinh, String maHuyen, String maXa,
		...
		try {
			count = VeSinhGiaDinhLocalServiceUtil.countVSGDByTHXMNN(maTinh,
					maHuyen, maXa, namLayMau);
		} catch (Exception e) {
			_log.error(e);
		}
