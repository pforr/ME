...
<%
	...
	List<TramCapNuoc> lisTramCap = TramCapBusiness.getByTinh(maTinh);
	
	DanhGiaBenVung danhGiaBenVung = null;
	
	danhGiaBenVung = DanhGiaBenVungBussiness.findBenVungById(danhGiaId);
...
<aui:form action="<%= addBenVungURL %>" method="post" name="fm">
	...
	<aui:field-wrapper cssClass="form-add-ben-vung">
		...
		<aui:select name="selTinh" onChange='<%= renderResponse.getNamespace() + "selectFilter()" %>'
			cssClass="sel-col-2" >
			...
			%>
				<aui:option selected="" value="<%= tinh.getNode1() %>"><%= tinh.getName() %></aui:option>
			<%
				}
			%>
		</aui:select>
