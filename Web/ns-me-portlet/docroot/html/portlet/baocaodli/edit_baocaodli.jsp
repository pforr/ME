...
<%
	...
	boolean deletePermission = baoCaoDaChot && BaoCaoDLIPermission.contains(permissionChecker, scopeGroupId, ActionKeys.ADD_BAOCAO);
...
<portlet:renderURL var="previewBaoCaoDLIURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
	...
	<aui:form action="" name="fm" method="post" onSubmit='<%= "event.preventDefault(); submitForm(event.target);" %>'>
		...
		<aui:fieldset>
			...
			<div style='<%= typeDLI != 5 ? "display: none;" : StringPool.BLANK %>'>
				<aui:row>
					<aui:col width="<%= 25 %>">
						<aui:input name="keHoachNam" disabled="<%= !updatePermission %>"/>
					</aui:col>
					
					<aui:col width="<%= 25 %>">
						<aui:input name="ngayPheDuyet" disabled="<%= !updatePermission %>" />
					</aui:col>
					
					<aui:col width="<%= 25 %>">
						<aui:input name="baoCaoChuongTrinh" disabled="<%= !updatePermission %>"/>
					</aui:col>
					
					<aui:col width="<%= 25 %>">
						<aui:input name="ngayDang" disabled="<%= !updatePermission %>"/>
					</aui:col>
				</aui:row>
			</div>
			
			<div style='<%= typeDLI != 10 ? "display: none;" : StringPool.BLANK %>'>
				<aui:row>
					<aui:col width="<%= 25 %>">
						...
						<aui:select name="giaiDoanTu">
							<%
								for (int i = 2014; i <= currYear; i++) {
							%>
					...
					<aui:col width="<%= 25 %>">
						...
						<%
							int currYear2 = Calendar.getInstance().get(Calendar.YEAR);
						%>
						<aui:select name="giaiDoanDen">
							<%
								for (int i = 2015; i <= currYear2; i++) {
							%>
								<aui:option value="<%= i %>"><%= i %></aui:option>
							<% 
								}
	...
	Liferay.provide(
		...
		function() {
			...
			if(previewBaoCaoId) {
				...
				var keHoachNam = A.one('#<portlet:namespace />keHoachNam');
				var ngayPheDuyet = A.one('#<portlet:namespace />ngayPheDuyet');
				var baoCaoChuongTrinh = A.one('#<portlet:namespace />baoCaoChuongTrinh');
				var ngayDang = A.one('#<portlet:namespace />ngayDang');
				var giaiDoanTu = A.one('#<portlet:namespace />giaiDoanTu');
				var giaiDoanDen = A.one('#<portlet:namespace />giaiDoanDen');
				...
				A.io.request(ajaxURL, { 
					data : { 
						...
						"<portlet:namespace />keHoachNam" : keHoachNam.val(),
						"<portlet:namespace />ngayPheDuyet" : ngayPheDuyet.val(),
						"<portlet:namespace />baoCaoChuongTrinh" : baoCaoChuongTrinh.val(),
						"<portlet:namespace />ngayDang" : ngayDang.val(),
						"<portlet:namespace />giaiDoanTu" : giaiDoanTu.val(),
						"<portlet:namespace />giaiDoanDen" : giaiDoanDen.val(),
					}, 
					on : { 
