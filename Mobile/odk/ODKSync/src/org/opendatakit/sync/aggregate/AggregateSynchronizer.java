...
public class AggregateSynchronizer implements Synchronizer {
	...
	private static final String LOGTAG = AggregateSynchronizer.class
			.getSimpleName();
	...
	// parameter for file downloads -- if we want to have it come down as an
	// attachment.
	...
	private final URI normalizeUri(String aggregateUri,
			String additionalPathPortion) {
	 ...
	 * string as for an html url, but leaves forward slashes. The path must
	 * begin with a forward slash, as if starting at the root directory.
	 * 
	...
	private String getTablesUriFragment() {
		/**
		 * Path to the tables servlet (the one that manages table definitions)
	...
	private String getFilePathURI() {
		return "/odktables/" + escapeSegment(appName) + "/files/"
				+ escapeSegment(odkClientApiVersion) + "/";
	}
	 ...
	 */
	private Resource buildBasicResource(URI uri)
			throws InvalidAuthTokenException {
		...
		rsc.header(ApiConstants.ACCEPT_CONTENT_ENCODING_HEADER,
				ApiConstants.GZIP_CONTENT_ENCODING);
		rsc.header(ApiConstants.OPEN_DATA_KIT_VERSION_HEADER,
				ApiConstants.OPEN_DATA_KIT_VERSION);
		rsc.header(HttpHeaders.USER_AGENT, "Sync "
				+ Sync.getInstance().getVersionCodeString() + " (gzip)");
	...
	private Resource buildResource(URI uri, MediaType contentType)
		...
		// set our preferred response media type to json using quality
		// parameters
	...
	public AggregateSynchronizer(Context context, String appName,
			String odkApiVersion, String aggregateUri, String accessToken)
			throws InvalidAuthTokenException {
	...
	public String updateAccessToken() throws InvalidAuthTokenException {
		...
		try {
			...
			this.accessToken = accountManager.blockingGetAuthToken(account,
					authString, true);
			return accessToken;
		} catch (Exception e) {
			e.printStackTrace();
			throw new InvalidAuthTokenException(
					"unable to update access token -- please re-authorize");
		}
	}
	...
	private void checkAccessToken(String accessToken)
		...
		try {
			...
			try {
				...
				throw new InvalidAuthTokenException(
						"Unable to parse response from auth token verification ("
								+ e.toString() + ")", e);
			}
			if (o != null && o instanceof Map) {
				...
				if (m.containsKey("error")) {
					throw new InvalidAuthTokenException("Invalid auth token ("
							+ m.get("error").toString() + "): " + accessToken,
							e);
				} else {
					throw new InvalidAuthTokenException(
							"Unknown response from auth token verification ("
									+ e.toString() + ")", e);
				}
			}
		} catch (Exception e) {
	...
	@Override
	public TableResourceList getTables(String webSafeResumeCursor)
		...
		try {
			...
			if (webSafeResumeCursor == null) {
				tableResources = buildResource(uri)
						.get(TableResourceList.class);
			} else {
				tableResources = buildResource(uri).queryParam(
						CURSOR_PARAMETER, webSafeResumeCursor).get(
						TableResourceList.class);
			}
		} catch (ClientWebException e) {
			log.e(LOGTAG,
					"Exception while requesting list of tables from server: "
							+ e.toString());
			throw e;
		}
	...
	public TableDefinitionResource getTableDefinition(String tableDefinitionUri)
		...
		TableDefinitionResource definitionRes = buildResource(uri).get(
	...
	public URI constructTableInstanceFileUri(String tableId, String schemaETag) {
		// e.g.,
		// https://msundt-test.appspot.com:443/odktables/tables/tables/Milk_bank/ref/uuid:bb26cdaf-9ccf-4a4f-8e28-c114fe30358a/attachments
		URI instanceFileUri = normalizeUri(aggregateUri, getTablesUriFragment()
				+ tableId + "/ref/" + schemaETag + "/attachments");
		return instanceFileUri;
	}

	@Override
	public TableResource createTable(String tableId, String schemaETag,
			ArrayList<Column> columns) throws ClientWebException,
			InvalidAuthTokenException {
		android.os.Debug.waitForDebugger();
		// build request
		URI uri = normalizeUri(aggregateUri, getTablesUriFragment() + tableId);
		TableDefinition definition = new TableDefinition(tableId, schemaETag,
				columns);
		// create table
		...
		try {
			...
			log.e(LOGTAG, "ResourceAccessException in createTable: " + tableId
	...
	public ChangeSetList getChangeSets(TableResource table, String dataETag)
		...
		try {
			...
			log.e(LOGTAG,
					"Exception while requesting list of changeSets from server: "
							+ tableId + " exception: " + e.toString());
			throw e;
		}
	}

	@Override
	public RowResourceList getChangeSet(TableResource table, String dataETag,
		...
		if (websafeResumeCursor != null) {
			resource = resource.queryParam(CURSOR_PARAMETER,
					websafeResumeCursor);
		}
		...
		try {
			...
			log.e(LOGTAG,
					"Exception while requesting changeSet rows from server: "
							+ tableId + " exception: " + e.toString());
			throw e;
		}
	}
	...
	public RowResourceList getUpdates(TableResource table, String dataETag,
			String websafeResumeCursor) throws ClientWebException,
			InvalidAuthTokenException {
		...
		try {
			...
			log.e(LOGTAG,
					"Exception while requesting list of rows from server: "
							+ tableId + " exception: " + e.toString());
			throw e;
		}
	}
	...
	public RowOutcomeList alterRows(TableResource resource,
		...
		for (SyncRow rowToAlter : rowsToInsertUpdateOrDelete) {
			Row row = Row.forUpdate(rowToAlter.getRowId(),
					rowToAlter.getRowETag(), rowToAlter.getFormId(),
					rowToAlter.getLocale(), rowToAlter.getSavepointType(),
					rowToAlter.getSavepointTimestamp(),
					rowToAlter.getSavepointCreator(),
					rowToAlter.getFilterScope(), rowToAlter.getValues());
		...
		try {
			...
			log.e(LOGTAG, "Exception while updating rows on server: "
					+ resource.getTableId() + " exception: " + e.toString());
			throw e;
		}
		return outcomes;
	}

	private static List<String> filterOutTableIdAssetFiles(
			List<String> relativePaths) {
		List<String> newList = new ArrayList<String>();
		for (String relativePath : relativePaths) {
			if (relativePath.startsWith("assets/csv/")) {
				// by convention, the files here begin with their identifying
				// tableId
				continue;
			} else {
	...
	/**
	 * Remove all assets/*.init files (e.g., tables.init) that only alter the
	 * data tables of the application. These are not needed on the server
	 * because their actions have already caused changes in the data tables that
	 * will be shared across all devices. I.e., they only need to be executed on
	 * the one starter device, and then everything gets replicated everywhere
	 * else.
	 * 
	 * @param relativePaths
	 * @return
	 */
	private static List<String> filterOutAssetInitFiles(
			List<String> relativePaths) {
		List<String> newList = new ArrayList<String>();
		for (String relativePath : relativePaths) {
	...
	private static List<String> filterInTableIdFiles(
			List<String> relativePaths, String tableId) {
		List<String> newList = new ArrayList<String>();
		for (String relativePath : relativePaths) {
			if (relativePath.startsWith("assets/csv/")) {
				// by convention, the files here begin with their identifying
	...
	private List<String> getAllFilesUnderFolder(File baseFolder,
		...
		if (!baseFolder.exists()) {
			...
			log.e(LOGTAG,
					"[getAllFilesUnderFolder] folder is not a directory: "
		...
		File[] partials = baseFolder.listFiles(new FileFilter() {
			...
			public boolean accept(File pathname) {
				if (excludingNamedItemsUnderFolder == null) {
					...
					return !excludingNamedItemsUnderFolder.contains(pathname
							.getName());
				}
			}
		});
	...
	@Override
	public boolean syncAppLevelFiles(boolean pushLocalFiles,
		...
		syncStatus.updateNotification(SyncProgressState.APP_FILES,
				R.string.getting_app_level_manifest, null, 1.0, false);
		List<OdkTablesFileManifestEntry> manifest = getAppLevelFileManifest(
				pushLocalFiles, serverReportedAppLevelETag);
		...
		Set<String> dirsToExclude = ODKFileUtils
				.getDirectoriesToExcludeFromSync(true);
		...
		double stepSize = 100.0 / (1 + relativePathsOnDevice.size() + manifest
				.size());
		int stepCount = 1;

		if (pushLocalFiles) {
			// if we are pushing, we want to push the local files that are
			// different
			// up to the server, then remove the files on the server that are
			// not
			// in the local set.
			List<String> serverFilesToDelete = new ArrayList<String>();

			for (OdkTablesFileManifestEntry entry : manifest) {
				File localFile = ODKFileUtils
						.asAppFile(appName, entry.filename);
				if (!localFile.exists() || !localFile.isFile()) {
					// we need to delete this file from the server.
					serverFilesToDelete.add(entry.filename);
				} else if (ODKFileUtils.getMd5Hash(appName, localFile).equals(
						entry.md5hash)) {
			...
			stepSize = 100.0 / (1 + relativePathsOnDevice.size() + serverFilesToDelete
					.size());

			for (String relativePath : relativePathsOnDevice) {

				syncStatus.updateNotification(SyncProgressState.APP_FILES,
						R.string.uploading_local_file,
						new Object[] { relativePath }, stepCount * stepSize,
						false);
				...
				if (!uploadFile(wholePathToFile, relativePath)) {
					success = false;
					log.e(LOGTAG, "Unable to upload file to server: "
							+ relativePath);
				}
			...
			for (String relativePath : serverFilesToDelete) {
				...
				syncStatus.updateNotification(SyncProgressState.APP_FILES,
						R.string.deleting_file_on_server,
						new Object[] { relativePath }, stepCount * stepSize,
						false);

				if (!deleteFile(relativePath)) {
					success = false;
					log.e(LOGTAG, "Unable to delete file on server: "
							+ relativePath);
				}
			...
			// if we are pulling, we want to pull the server files that are
			// different
			// down from the server, then remove the local files that are not
			// present
			// on the server.

			for (OdkTablesFileManifestEntry entry : manifest) {
				syncStatus.updateNotification(SyncProgressState.APP_FILES,
						R.string.verifying_local_file,
						new Object[] { entry.filename }, stepCount * stepSize,
						false);

				// make sure our copy is current
				compareAndDownloadFile(null, entry);
				// remove it from the set of app-level files we found before the
				// sync
				relativePathsOnDevice.remove(entry.filename);

				// this is the corrected step size based upon matching files
				stepSize = 100.0 / (1 + relativePathsOnDevice.size() + manifest
						.size());
			...
			for (String relativePath : relativePathsOnDevice) {
				...
				syncStatus.updateNotification(SyncProgressState.APP_FILES,
						R.string.deleting_local_file,
						new Object[] { relativePath }, stepCount * stepSize,
						false);

				// and remove any remaining files, as these do not match
				// anything on
				// the server.
	...
	@Override
	public void syncTableLevelFiles(String tableId,
			String serverReportedTableLevelETag,
			OnTablePropertiesChanged onChange, boolean pushLocalFiles,
			SynchronizerStatus syncStatus) throws ClientWebException,
			InvalidAuthTokenException {
		android.os.Debug.waitForDebugger();
		syncStatus.updateNotification(SyncProgressState.TABLE_FILES,
				R.string.getting_table_manifest, new Object[] { tableId }, 1.0,
				false);

		// get the table files on the server
		List<OdkTablesFileManifestEntry> manifest = getTableLevelFileManifest(
				tableId, serverReportedTableLevelETag, pushLocalFiles);

		if (manifest == null) {
			log.i(LOGTAG, "no change in table manifest -- skipping!");
			// short-circuit because our files should match those on the server
			syncStatus.updateNotification(SyncProgressState.TABLE_FILES,
					R.string.getting_table_manifest, new Object[] { tableId },
					100.0, false);

			return;
		}

		String tableIdPropertiesFile = "tables" + File.separator + tableId
		...
		File assetsCsvFolder = new File(ODKFileUtils.getAssetsFolder(appName)
				+ "/csv");
		...
		// We don't want to sync anything in the instances directory, because
		// this
		// contains things like media attachments.
		File tableFolder = new File(ODKFileUtils.getTablesFolder(appName,
				tableId));
		...
		if (pushLocalFiles) {
			...
			for (OdkTablesFileManifestEntry entry : manifest) {
				...
				// and if it was the table properties file, remember whether it
				// changed.
	...
	public List<OdkTablesFileManifestEntry> getAppLevelFileManifest(
			boolean pushLocalFiles, String serverReportedAppLevelETag)
			throws ClientWebException, InvalidAuthTokenException {
		SyncETagsUtils seu = new SyncETagsUtils();
		URI fileManifestUri = normalizeUri(aggregateUri,
				getManifestUriFragment());
		String eTag = seu.getManifestSyncETag(context, appName,
				fileManifestUri, null);
		// android.os.Debug.waitForDebugger();
		...
		if (!pushLocalFiles && eTag != null) {
			...
			if (serverReportedAppLevelETag != null
					&& serverReportedAppLevelETag.equals(eTag)) {
		...
		if (!rsp.getHeaders().containsKey(
				ApiConstants.OPEN_DATA_KIT_VERSION_HEADER)) {
			throw new ClientWebException(null, rsp);
		}
		...
		seu.updateManifestSyncETag(context, appName, fileManifestUri, null,
				eTag);
		// and return the list of values...
		return theList;
	}

	public List<OdkTablesFileManifestEntry> getTableLevelFileManifest(
			String tableId, String serverReportedTableLevelETag,
			boolean pushLocalFiles) throws ClientWebException,
			InvalidAuthTokenException {
		
		SyncETagsUtils seu = new SyncETagsUtils();
		URI fileManifestUri = normalizeUri(aggregateUri,
				getManifestUriFragment() + tableId);
		String eTag = seu.getManifestSyncETag(context, appName,
				fileManifestUri, tableId);
		Resource rsc = buildResource(fileManifestUri);
		// don't short-circuit manifest if we are pushing local files,
		// as we need to know exactly what is on the server to minimize
		// transmissions of files being pushed up to the server.
		if (!pushLocalFiles && eTag != null) {
			...
			if (serverReportedTableLevelETag != null
					&& serverReportedTableLevelETag.equals(eTag)) {
				// no change -- we can skip the request to the server
				return null;
			}
		}
		...
		// update the manifest ETag record...
		eTag = rsp.getHeaders().getFirst(HttpHeaders.ETAG);
		seu.updateManifestSyncETag(context, appName, fileManifestUri, tableId,
				eTag);
		// and return the list of values...
		return theList;
	}
	...
	private boolean deleteFile(String pathRelativeToAppFolder)
		...
		URI filesUri = normalizeUri(aggregateUri, getFilePathURI()
				+ escapedPath);
	...
	private boolean uploadFile(String wholePathToFile,
			String pathRelativeToAppFolder) throws InvalidAuthTokenException {
		...
		// android.os.Debug.waitForDebugger();
		ClientResponse response = buildResource(filesUri, contentType).post(
				file);
	...
	private boolean uploadInstanceFile(File file, URI instanceFileUri)
		...
		MediaType contentType = MediaType.valueOf(ct);
		// android.os.Debug.waitForDebugger();
		ClientResponse response = buildResource(instanceFileUri, contentType)
				.post(file);
		if (response.getStatusCode() < 200 || response.getStatusCode() >= 300) {
			return false;
		}
	 ...
	 * @return
	 */
	private boolean compareAndDownloadFile(String tableId,
		...
		if (entry.filename == null || entry.filename.equals("")) {
			...
			URL urlFile = null;
			try {
				log.i(LOGTAG, "[downloadFile] downloading at url: "
						+ entry.downloadUrl);
			...
			if (!newFile.exists()) {
				...
				try {
					...
					if (statusCode == HttpStatus.SC_OK) {
						seu.updateFileSyncETag(context, appName, uri, tableId,
								newFile.lastModified(), entry.md5hash);
						return true;
					} else {
						return false;
					}
				...
				String md5hash = seu.getFileSyncETag(context, appName, uri,
						tableId, newFile.lastModified());
				if (md5hash == null) {
				...
				if (!md5hash.equals(entry.md5hash)) {
					...
					try {
						...
						if (statusCode == HttpStatus.SC_OK
							...
							seu.updateFileSyncETag(context, appName, uri,
									tableId, newFile.lastModified(), md5hash);
							return true;
						} else {
							return false;
						}
						...
						log.e(LOGTAG,
								"trouble downloading new version of existing file");
						return false;
					}
				} else {
					if (!hasUpToDateEntry) {
						seu.updateFileSyncETag(context, appName, uri, tableId,
								newFile.lastModified(), md5hash);
					}
					// no change
					return false;
				}
		...
		while (!success && attemptCount++ <= 2) {
			...
			try {
				...
				if (statusCode != HttpStatus.SC_OK) {
					...
					log.w(LOGTAG, "downloading " + downloadUrl.toString()
							+ " returns " + statusCode);
					return statusCode;
				}
				...
				if (!response.getHeaders().containsKey(
					...
					response.consumeContent();
					log.w(LOGTAG, "downloading " + downloadUrl.toString()
							+ " appears to have been redirected.");
					return 302;
				}

				File tmp = new File(destFile.getParentFile(),
				...
				try {
					...
					e.printStackTrace();
					log.e(LOGTAG,
							"downloading " + downloadUrl.toString()
									+ " failed after " + totalLen + " bytes: "
									+ e.toString());
					try {
	...
	private CommonFileAttachmentTerms computeCommonFileAttachmentTerms(
			String instanceFileUri, String instanceId, File instanceFolder,
			String relativePath) {
		// clean up the value...
		...
		String relativeInstanceFolderPath = ODKFileUtils.asRelativePath(
				appName, instanceFolder);
		File localFile;
		...
		if (!baseLocalAttachment.startsWith(baseInstanceFolder)) {
			throw new IllegalStateException(
					"instance data file is not within the instances tree!");
		}
		String partialValue = baseLocalAttachment.substring(baseInstanceFolder
				.length());
		if (partialValue.startsWith("/")) {
			partialValue = partialValue.substring(1);
		}

		URI instanceFileDownloadUri = normalizeUri(instanceFileUri, instanceId
				+ "/file/" + partialValue);
	...
	@Override
	public boolean getFileAttachments(String instanceFileUri, String tableId,
			SyncRowPending serverRow, boolean deferInstanceAttachments)
			throws ClientWebException {

		if (serverRow.getUriFragments().isEmpty()) {
			this.log.w("getFileAttachments",
					"logic error - probably from conflict mgmt - should never get here!");
			return true;
		}
		 ...
		 * 
		 * 
		 * With the new mechanism, we can directly fetch any file from the
		 * server in the fileAttachmentColumns. And we can include any md5Hash
		 * we might have of the local file. This would enable the server to say
		 * "yes, it is identical".
		 */
		boolean success = true;
		try {
			...
			File instanceFolder = new File(ODKFileUtils.getInstanceFolder(
			...
			for (String relativePath : serverRow.getUriFragments()) {
				...
				CommonFileAttachmentTerms cat = computeCommonFileAttachmentTerms(
						instanceFileUri, instanceId, instanceFolder,
						relativePath);

				String appFolderRelativePath = ODKFileUtils.asRelativePath(
						appName, cat.localFile);
				...
				if (!cat.localFile.exists()) {
					...
					int statusCode = downloadFile(cat.localFile,
							cat.instanceFileDownloadUri);
					if (statusCode == HttpStatus.SC_OK
							|| statusCode == HttpStatus.SC_NOT_MODIFIED) {
						String md5Hash = ODKFileUtils.getMd5Hash(appName,
								cat.localFile);
						seu.updateFileSyncETag(context, appName,
								cat.instanceFileDownloadUri, tableId,
								cat.localFile.lastModified(), md5Hash);
					} else {
						success = false;
					}
				} else {
					// assume that if the local file exists, it matches exactly
					// the
					// content on the server.
					// this could be inaccurate if there are buggy programs on
					// the
					// device!
					String md5hash = seu.getFileSyncETag(context, appName,
							cat.instanceFileDownloadUri, tableId,
							cat.localFile.lastModified());
					if (md5hash == null) {
						md5hash = ODKFileUtils.getMd5Hash(appName,
								cat.localFile);
						seu.updateFileSyncETag(context, appName,
								cat.instanceFileDownloadUri, tableId,
								cat.localFile.lastModified(), md5hash);
			...
			if (serverRow.shouldDeleteExtraneousLocalFiles()) {
				for (String relativePath : relativePathsToAppFolderOnDevice) {
					...
					File localFile = ODKFileUtils.asAppFile(appName,
							relativePath);
					if (!localFile.delete()) {
						success = false;
					}
	...
	@Override
	public boolean putFileAttachments(String instanceFileUri, String tableId,
			SyncRowPending localRow, boolean deferInstanceAttachments)
			throws ClientWebException {

		if (localRow.getUriFragments().isEmpty()) {
			this.log.w("putFileAttachments",
					"logic error - probably from conflict mgmt - should never get here!");
			return true;
		}
		 ...
		 * 
		 * With the new mechanism, we can directly fetch any file from the
		 * server in the fileAttachmentColumns. For PUT, we don't know if the
		 * local file exists on the server, so we need to PUT every attachment.
		 * This can be done by retrieving the manifest, and comparing that
		 * against the local directory, or we can issue an if-none-match GET
		 * request for each file we need. If we do not get a NOT_MODIFIED
		 * return, then we upload it.
		 */
		boolean success = true;
		try {
			// 1) Get this row's instanceId (rowId)
			...
			for (String relativePath : localRow.getUriFragments()) {
				...
				if (cat.localFile.exists()) {
					...
					// issue a GET. If the return is NOT_MODIFIED, then we don't
					// need to
					// POST it.
					int statusCode = downloadFile(cat.localFile,
							cat.instanceFileDownloadUri);
					if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
						...
						// The test for ODK header ensures we detect wifi login
						// overlays
						// this should not happen -- indicates something is
						// corrupted.
						log.e(LOGTAG, "Unexpectedly overwriting attachment: "
								+ cat.instanceFileDownloadUri.toString());
						...
						// upload it...
						boolean outcome = uploadInstanceFile(cat.localFile,
								cat.instanceFileDownloadUri);
						if (!outcome) {
							success = false;
						}
					...
					// we will pull these files later during a
					// getFileAttachments() call,
					// if needed...
				}
			}
			return success;
