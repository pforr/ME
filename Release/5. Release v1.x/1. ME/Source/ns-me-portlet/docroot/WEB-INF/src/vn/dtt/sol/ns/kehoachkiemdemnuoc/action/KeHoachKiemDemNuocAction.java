...
public class KeHoachKiemDemNuocAction extends MVCPortlet {
	...
	private void updateStatusDauNoi(THONGTINKIEMDEMNUOC thongtinkiemdemnuoc,
		...
		if(type == 1){ // xoa kehoach
			ettUpdate.setTrangThai(0); // daunoinuoc trang thai chuyen ve chua kiemdem
		}else if(type == 2){ // dong ke hoach
			if (thongtinkiemdemnuoc.getDaKetThucDieuTra() == 0) { //  dangchokhaosat
				thongtinkiemdemnuoc.setDaKetThucDieuTra(3); // dau noi nuoc trong ke hoach thanh DAHUY
				ettUpdate.setTrangThai(0); // daunoinuoc trang thai chuyen ve chua kiemdem
			}
			if (thongtinkiemdemnuoc.getDanhGiaKiemDem() == 1) { // danh gia la dat
				ettUpdate.setTrangThai(2); // daunoinuoc se chuyen thanh dat
			} else if (thongtinkiemdemnuoc.getDanhGiaKiemDem() == 2) { // danh gia la ko dat
				ettUpdate.setTrangThai(3); // daunoinuoc se chuyen thanh khong dat
			}
		}
		/*
