...
public class NavigationBusiness {
	...
	public static List<NAVIGATION> getLeftMenu(int parentId, int hidden, String currentUrl, int initUserType,int role,String orgCode, String permission) {
		
		List<NAVIGATION> resutlsBase = new ArrayList<NAVIGATION>();
		List<NAVIGATION> resutls = new ArrayList<NAVIGATION>();
		try {
			resutlsBase = NAVIGATIONLocalServiceUtil.getLeftMenu(parentId,hidden);
			long sumTongHop = 0;
			for (NAVIGATION navigation : resutlsBase) {
				if(currentUrl.contains(navigation.getNavSubOwner())){
					sumTongHop = DauNoiNuocLocalServiceUtil.countThongKeMenuLeft(navigation.getThongKe(), initUserType, orgCode);
