...
import vn.dtt.sol.ns.reporting.util.CSV_KeHoachExport;
...
public class KeHoachKiemDemVeSinhAction extends MVCPortlet{
	...
	public void doView(RenderRequest renderRequest,
		...
		String maTinh = ParamUtil.getString(renderRequest, KeHoachKiemDemVeSinhTerms.MA_TINH);
		String maHuyen = ParamUtil.getString(renderRequest, KeHoachKiemDemVeSinhTerms.MA_HUYEN);
		String kiemDemVienId = ParamUtil.getString(renderRequest, KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID);
...
//		List<DATAITEM> listXa = new ArrayList<DATAITEM>();
		List<KiemDemVien>  listKiemDemVien = new ArrayList<KiemDemVien>();
		 try {
			...
			int id = ParamUtil.getInteger(renderRequest, KeHoachKiemDemVeSinhTerms.ID);
			if(id>0){
		...
		renderRequest.setAttribute(KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID, kiemDemVienId);
	...
	public void render(RenderRequest request, RenderResponse response)
		...
		User user = null;
		UserMapping loginUserMapping = new UserMappingImpl();
		String maTinh = ParamUtil.getString(request, KeHoachKiemDemVeSinhTerms.MA_TINH);
		String maHuyen = ParamUtil.getString(request, KeHoachKiemDemVeSinhTerms.MA_HUYEN);
		String kiemDemVienId = ParamUtil.getString(request, KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID);
		...
		List<DATAITEM> listLoaiCongTrinh = new ArrayList<DATAITEM>();
		 try {
			...
			if(id>0){
				keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhBussiness.fetchKeHoachKiemDemVeSinh(id);
				request.setAttribute("keHoachKiemDemVeSinh", keHoachKiemDemVeSinh);
				listHuyen = DataItemBusiness.getInLevel2(WebKeys.DATAGROUPID_DONVIHANHCHINH, keHoachKiemDemVeSinh.getMaTinh());
			}
			if(loginUserMapping.getUserType()==DauNoiNuocConstantString.UserMapping.CAP_TU){
				listTinh = DATAITEMLocalServiceUtil.getByDataGroupLevel1(WebKeys.DATAGROUPID_DONVIHANHCHINH);
				if(Validator.isNotNull(maTinh)){
					...
					if(Validator.isNotNull(maHuyen)){
						listKiemDemVien = KiemDemVienBussiness.findByCapHoatDongOrgCode(String.valueOf(WebKeys.KDV_VSHUYEN), maHuyen);
					}else{
						listKiemDemVien = KiemDemVienBussiness.findByCapHoatDongOrgCode(String.valueOf(WebKeys.KDV_TOAN_TINH), maTinh);
					}
			...
			if(Validator.isNotNull(keHoachKiemDemVeSinh)){
				...
				listKiemDemVien = KiemDemVienBussiness.findByCapHoatDongOrgCode(String.valueOf(WebKeys.KDV_VSHUYEN), keHoachKiemDemVeSinh.getMaHuyen());
			}
			listLoaiCongTrinh = DataItemBusiness.getByDataGroupId(WebKeys.DATAGROUPID_LOAICONGTRINH);
		} catch (PortalException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (SystemException e) {
		...
		request.setAttribute(KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID, kiemDemVienId);
	...
	public void serveResource(ResourceRequest resourceRequest,
		...
		int idKeHoach = ParamUtil.getInteger(resourceRequest, KeHoachKiemDemVeSinhTerms.ID);
...
//			CSV_KeHoachExport.writeCsvFile(resourceRequest,resourceResponse);
	...
	private void xuatBankeHoachKiemDem(ResourceRequest resourceRequest,ResourceResponse resourceResponse,int idKeHoach){
		...
		HttpServletResponse respon = PortalUtil.getHttpServletResponse(resourceResponse);
		try {
			keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhLocalServiceUtil.fetchKeHoachKiemDemVeSinh(idKeHoach);
			if(Validator.isNotNull(keHoachKiemDemVeSinh)){
				keHoachKiemDemVeSinh.setNgayXuatBan(new Date());
			}
			KeHoachKiemDemVeSinhLocalServiceUtil.updateKeHoachKiemDemVeSinh(keHoachKiemDemVeSinh);
	...
	public void xuatBankeHoachKiemDem(ActionRequest actionRequest,
		...
		KeHoachKiemDemVeSinh keHoachKiemDemVeSinh = new KeHoachKiemDemVeSinhImpl();
		try {
			int idKeHoach = ParamUtil.getInteger(actionRequest, KeHoachKiemDemVeSinhTerms.ID);
			keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhLocalServiceUtil.fetchKeHoachKiemDemVeSinh(idKeHoach);
			if(Validator.isNotNull(keHoachKiemDemVeSinh)){
				keHoachKiemDemVeSinh.setNgayXuatBan(new Date());
			}
	...
	public void addkeHoachKiemDem(ActionRequest actionRequest,
		...
		long idReturn = 0;
		KeHoachKiemDemVeSinhTerms keHoachKiemDemVeSinhTerms = new KeHoachKiemDemVeSinhTerms(actionRequest);
		String add = "";
		...
		try {
			...
			if (add.equals(Constants.ADD)) {
				long id = 0;
				id = CounterLocalServiceUtil.increment(KeHoachKiemDemVeSinh.class.getName());
				keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhLocalServiceUtil.createKeHoachKiemDemVeSinh(Integer.valueOf(id+""));
				keHoachKiemDemVeSinh.setIdNguoiTao(Integer.valueOf(String.valueOf(userId)));
				...
				keHoachKiemDemVeSinh = KeHoachKiemDemVeSinhLocalServiceUtil.fetchKeHoachKiemDemVeSinh(keHoachKiemDemVeSinhTerms.getId());
				messageKey = MessageSuccess.EDIT_KE_HOACH_KIEM_DEM_MESSAGE;
				idReturn=keHoachKiemDemVeSinhTerms.getId();
			}
			
			keHoachKiemDemVeSinh.setMaTinh(keHoachKiemDemVeSinhTerms.getMaTinh());
			keHoachKiemDemVeSinh.setMaHuyen(keHoachKiemDemVeSinhTerms.getMaHuyen());
			keHoachKiemDemVeSinh.setKiemDemVienID(keHoachKiemDemVeSinhTerms.getKiemDemVienId());
			keHoachKiemDemVeSinh.setNgayBatDau(keHoachKiemDemVeSinhTerms.getNgayBatDau());
			keHoachKiemDemVeSinh.setNgayKetThuc(keHoachKiemDemVeSinhTerms.getNgayKetThuc());
			keHoachKiemDemVeSinh.setMoTa(keHoachKiemDemVeSinhTerms.getMoTa());
...
//			actionResponse.sendRedirect(cuString);
	...
	private void updateStatusHoGiaDinh(ThongTinKiemDemHoGD thongtinkiemdemhogd, int type) throws SystemException {
		// TODO Auto-generated method stub
		VeSinhGiaDinh ettUpdate = new VeSinhGiaDinhImpl();
		ettUpdate = VeSinhGiaDinhLocalServiceUtil.fetchVeSinhGiaDinh(thongtinkiemdemhogd.getVeSinhGiaDinhId());
		...
		ThongTinKiemDemHoGDLocalServiceUtil.updateThongTinKiemDemHoGD(thongtinkiemdemhogd);
		VeSinhGiaDinhLocalServiceUtil.updateVeSinhGiaDinh(ettUpdate);
	}
	private void updateStatusCongTrinh(ThongTinKiemDemCongTrinh thongtinkiemdemcongtrinh, int type) throws SystemException {
		// TODO Auto-generated method stub
		VeSinhCongTrinh ettUpdate = new VeSinhCongTrinhImpl();
		ettUpdate = VeSinhCongTrinhLocalServiceUtil.fetchVeSinhCongTrinh(thongtinkiemdemcongtrinh.getVeSinhConTrinhId());
		...
		ThongTinKiemDemCongTrinhLocalServiceUtil.updateThongTinKiemDemCongTrinh(thongtinkiemdemcongtrinh);
	...
	public void themAllDauNoiToKeHoachKieDemVesinh(ActionRequest actionRequest,
		...
		long kehoachId = ParamUtil.getLong(actionRequest, KeHoachKiemDemVeSinhTerms.ID);
		KeHoachKiemDemVeSinh keHoach = KeHoachKiemDemVeSinhBussiness.fetchKeHoachKiemDemVeSinh(Integer.valueOf(kehoachId+""));
		List<VeSinhGiaDinh> listHoGdByOrgId = new ArrayList<VeSinhGiaDinh>();
		idReturn = maDotBaoCao;
		_log.info("keHoach.getMaTinh()"+keHoach.getMaTinh()+""+keHoach.getMaHuyen());
		listHoGdByOrgId = VeSinhGiaDinhLocalServiceUtil.getVeSinhGiaDinhSearch(keHoach.getMaTinh(),keHoach.getMaHuyen(), "", "", "0", "9999"+(Validator.isNotNull(keHoach)?keHoach.getNam():StringPool.BLANK),"", QueryUtil.ALL_POS, QueryUtil.ALL_POS);
	...
	public void themDauNoiToKeHoachKieDemVesinhHoGDId(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		String messageKey = MessageErrors.THEM_GIA_DINH_KE_HOACH_MESSAGE;
		long idReturn = 0;
		try {
		 ...
		 for (long l : orgIds) {
			 ettGiaDinh = VeSinhGiaDinhLocalServiceUtil.fetchVeSinhGiaDinh(Integer.valueOf(l+""));
			 if(trangThai == 1){
				 listHoGdByOrgId.add(VeSinhGiaDinhLocalServiceUtil.fetchVeSinhGiaDinh(Integer.valueOf(l+"")));
			 }else if(Validator.isNotNull(ettGiaDinh) && (ettGiaDinh.getTrangThai() == 0 ||  ettGiaDinh.getTrangThai() == 8)){
				 listHoGdByOrgId.add(VeSinhGiaDinhLocalServiceUtil.fetchVeSinhGiaDinh(Integer.valueOf(l+"")));
			 }
		}
		 doInsertHoGdToKeHoach(listHoGdByOrgId,idReturn);
	...
	public void themAllCongTrinhToKeHoachKieDemVesinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		...
		idReturn = maDotBaoCao;
		_log.info("keHoach.getMaTinh()"+keHoach.getMaTinh()+""+keHoach.getMaHuyen());
		listCongTrinhByOrgId = VeSinhCongTrinhLocalServiceUtil.getVeSinhCongTrinhSearch(keHoach.getMaTinh(), 
																					keHoach.getMaHuyen(), 
																					"", 
																					"", 
																					"", 
																					"0", 
																					"9999"+(Validator.isNotNull(keHoach)?keHoach.getNam():StringPool.BLANK),
	...
	public void themCongTrinhToKeHoachKieDemNVesinh(ActionRequest actionRequest,
			ActionResponse actionResponse) throws IOException {
		boolean isSuccess = false;
		...
		try {
		 ...
		 for (long l : orgIds) {
			 ettCongTrinh = VeSinhCongTrinhLocalServiceUtil.fetchVeSinhCongTrinh(Integer.valueOf("0"+l));
			 if(trangThai == 1){
				 listCongTrinhByOrgId.add(VeSinhCongTrinhLocalServiceUtil.fetchVeSinhCongTrinh(Integer.valueOf(l+"")));
			 }else if(Validator.isNotNull(ettCongTrinh) && (ettCongTrinh.getTrangThai() == 0 ||  ettCongTrinh.getTrangThai() == 8)){
				 listCongTrinhByOrgId.add(VeSinhCongTrinhLocalServiceUtil.fetchVeSinhCongTrinh(Integer.valueOf(l+"")));
			 }
		}
		 doInsertCongTrinhToKeHoach(listCongTrinhByOrgId,idReturn);
	...
	private void doInsertHoGdToKeHoach(List<VeSinhGiaDinh> listHoGdByOrgId, long keHoachId) throws SystemException {
		// TODO Auto-generated method stub
		ThongTinKiemDemHoGD ettInsert = new ThongTinKiemDemHoGDImpl();
		KeHoachKiemDemVeSinh keHoach = null;
		keHoach = KeHoachKiemDemVeSinhLocalServiceUtil.fetchKeHoachKiemDemVeSinh(Integer.valueOf(keHoachId+""));
		...
		for (VeSinhGiaDinh veSinhGiaDinh : listHoGdByOrgId) {
			...
			long id=0;
			id = CounterLocalServiceUtil.increment(ThongTinKiemDemHoGD.class.getName());
			ettInsert = ThongTinKiemDemHoGDLocalServiceUtil.createThongTinKiemDemHoGD(Integer.valueOf(id+""));
			ettInsert.setVeSinhGiaDinhId(veSinhGiaDinh.getId());
			ettInsert.setKeHoachKiemDemVeSinhId(Integer.valueOf(keHoachId+""));
			...
			ThongTinKiemDemHoGDLocalServiceUtil.updateThongTinKiemDemHoGD(ettInsert);
		}
	}
	public void  phanCongLaiKeHoachKieDemCTHuyBo(ActionRequest actionRequest,
		...
		try {
		 ...
		 ThongTinKiemDemCongTrinh ettThongTin = null;
		 	for (long l : orgIds) {
		 		ettThongTin = ThongTinKiemDemCongTrinhLocalServiceUtil.fetchThongTinKiemDemCongTrinh(Integer.valueOf(l+""));
				_log.info("*****"+l);
				ettRevert = new VeSinhCongTrinhImpl();
				if(Validator.isNotNull(ettThongTin)){
					ettRevert = VeSinhCongTrinhLocalServiceUtil.fetchVeSinhCongTrinh(ettThongTin.getVeSinhConTrinhId());
					ettRevert.setTrangThai(8);
					VeSinhCongTrinhLocalServiceUtil.updateVeSinhCongTrinh(ettRevert);
				}
				ettThongTin.setDaKetThucDieuTra(3);
				ThongTinKiemDemCongTrinhLocalServiceUtil.updateThongTinKiemDemCongTrinh(ettThongTin);
			}
	...
	public void  phanCongLaiKeHoachKieDemGDHuyBo(ActionRequest actionRequest,
		...
		try {
		 ...
		 ThongTinKiemDemHoGD ettThongTin = null;
		 	for (long l : orgIds) {
		 		ettThongTin = ThongTinKiemDemHoGDLocalServiceUtil.fetchThongTinKiemDemHoGD(Integer.valueOf(l+""));
				_log.info("*****"+l);
				ettRevert = new VeSinhGiaDinhImpl();
				if(Validator.isNotNull(ettThongTin)){
					ettRevert = VeSinhGiaDinhLocalServiceUtil.fetchVeSinhGiaDinh(ettThongTin.getVeSinhGiaDinhId());
					ettRevert.setTrangThai(8);
					VeSinhGiaDinhLocalServiceUtil.updateVeSinhGiaDinh(ettRevert);
				}
				ettThongTin.setDaKetThucDieuTra(3);
				ThongTinKiemDemHoGDLocalServiceUtil.updateThongTinKiemDemHoGD(ettThongTin);
			}
		...
		try {
		 ...
		 ThongTinKiemDemHoGD ettThongTin = null;
		 	for (long l : orgIds) {
				...
				ThongTinKiemDemHoGDLocalServiceUtil.deleteThongTinKiemDemHoGD(Integer.valueOf(l+""));
			}
		 	messageKey = MessageSuccess.PHAN_CONG_LAI_GIA_DINH_MESSAGE;
			isSuccess = true;
		} catch (Exception e) {
	...
	private void doInsertCongTrinhToKeHoach(List<VeSinhCongTrinh> listCongTrinhByOrgId, long keHoachId) throws SystemException {
		// TODO Auto-generated method stub
		...
		for (VeSinhCongTrinh veSinhCongTrinh : listCongTrinhByOrgId) {
			...
			long id=0;
			id = CounterLocalServiceUtil.increment(ThongTinKiemDemCongTrinh.class.getName());
			ettInsert = ThongTinKiemDemCongTrinhLocalServiceUtil.createThongTinKiemDemCongTrinh(Integer.valueOf(id+""));
			...
			ThongTinKiemDemCongTrinhLocalServiceUtil.updateThongTinKiemDemCongTrinh(ettInsert);
		}
	}
		...
		try {
		 ...
		 ThongTinKiemDemCongTrinh ettThongTin = null;
		 	for (long l : orgIds) {
				...
				ThongTinKiemDemCongTrinhLocalServiceUtil.deleteThongTinKiemDemCongTrinh(Integer.valueOf(l+""));
			}
		 	messageKey = MessageSuccess.PHAN_CONG_LAI_CONG_TRINH_MESSAGE;
			isSuccess = true;
		} catch (Exception e) {
	...
	public void deleteKeHoachKiemDem(ActionRequest actionRequest,
		...
		try {
			...
			if(id>0){
				...
				keHoachKiemDemVeSinh.setIdNguoiDong(Integer.valueOf(userId+""));
//			//update 
				KeHoachKiemDemVeSinhLocalServiceUtil.updateKeHoachKiemDemVeSinh(keHoachKiemDemVeSinh);
//			
			List<ThongTinKiemDemHoGD> listTt = new ArrayList<ThongTinKiemDemHoGD>();
			listTt = KeHoachKiemDemVeSinhLocalServiceUtil.getListThongTinGdByKeHoachId(id);
			List<ThongTinKiemDemCongTrinh> listCt = new ArrayList<ThongTinKiemDemCongTrinh>();
			listCt = KeHoachKiemDemVeSinhLocalServiceUtil.getListThongTinCongTringByKeHoachId(id);
//			
			...
			KeHoachKiemDemVeSinhLocalServiceUtil.deleteKeHoachKiemDemVeSinh(id);
...
//				SessionMessages.add(actionRequest, PortalUtil.getPortletId(actionRequest) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_SUCCESS_MESSAGE);
				String successURL = ParamUtil.getString(actionRequest, "onpageSuccess");
				successURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.MA_TINH+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.MA_TINH);
				successURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.MA_HUYEN+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.MA_HUYEN);
				successURL+="&"+actionResponse.getNamespace()+"maXa="+ParamUtil.getString(actionRequest, "maXa");
				successURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID);
...
//				SessionMessages.add(actionRequest, PortalUtil.getPortletId(actionRequest) + SessionMessages.KEY_SUFFIX_HIDE_DEFAULT_ERROR_MESSAGE);	
				...
				errorURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.MA_TINH+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.MA_TINH);
				errorURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.MA_HUYEN+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.MA_HUYEN);
				errorURL+="&"+actionResponse.getNamespace()+"maXa="+ParamUtil.getString(actionRequest, "maXa");
				errorURL+="&"+actionResponse.getNamespace()+KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID+"="+ParamUtil.getString(actionRequest, KeHoachKiemDemVeSinhTerms.KIEM_DEM_VIEN_ID);
		...
		public void dongkeHoachKiemDem(ActionRequest actionRequest,
			...
			int id = 0;
			try {
				id = ParamUtil.getInteger(actionRequest, KeHoachKiemDemVeSinhTerms.ID);
				KeHoachKiemDemVeSinh keHoachKiemDemVeSinh = new KeHoachKiemDemVeSinhImpl();
...
//			
		...
		private static Log _log = LogFactoryUtil.getLog(KeHoachKiemDemVeSinhAction.class);
}
