...
<%
	...
	listStr = KeHoachKiemDemVeSinhLocalServiceUtil.getHuyenXaKeHoachKiemDemCongTrinhSearch(String.valueOf(keHoachKiemDemVeSinh.getId()), trangThai, keHoachKiemDemVeSinh.getMaTinh(), QueryUtil.ALL_POS, QueryUtil.ALL_POS);

	if(listStr.size()> 0 &&Validator.isNotNull(KeHoachKiemDemVeSinhBussiness.getHuyenKeHoachKiemDemVeSinhCongTrinhSearch(String.valueOf(keHoachKiemDemVeSinh.getId()), trangThai, keHoachKiemDemVeSinh.getMaTinh(), QueryUtil.ALL_POS, QueryUtil.ALL_POS).get(0))){
		listHuyen.addAll(KeHoachKiemDemVeSinhBussiness.getHuyenKeHoachKiemDemVeSinhCongTrinhSearch(String.valueOf(keHoachKiemDemVeSinh.getId()), trangThai, keHoachKiemDemVeSinh.getMaTinh(), QueryUtil.ALL_POS, QueryUtil.ALL_POS) );
	}
	if(listStr.size()> 0 &&Validator.isNotNull(KeHoachKiemDemVeSinhBussiness.getXaKeHoachKiemDemVeSinhCongTrinhSearch(String.valueOf(keHoachKiemDemVeSinh.getId()), trangThai, keHoachKiemDemVeSinh.getMaTinh(), QueryUtil.ALL_POS, QueryUtil.ALL_POS).get(0))){
		listXa.addAll(KeHoachKiemDemVeSinhBussiness.getXaKeHoachKiemDemVeSinhCongTrinhSearch(String.valueOf(keHoachKiemDemVeSinh.getId()), trangThai, keHoachKiemDemVeSinh.getMaTinh(), QueryUtil.ALL_POS, QueryUtil.ALL_POS) );
	}
...
<liferay-portlet:actionURL var="phanCongLaiKeHoachKieDemHoGdURL" name="phanCongLaiKeHoachKieDemCongTrinh" />
<liferay-portlet:actionURL var="phanCongLaiKeHoachKieDemCTHuyBoURL" name="phanCongLaiKeHoachKieDemCTHuyBo" />
<aui:form action="<%= searchURL %>" method="post" name="fm">
<aui:script>
Liferay.provide(
window,
'<portlet:namespace />revertCongTrinh',
 function() {
                    ...
                    if(checkBoxValue==""||checkBoxValue==null){
                            alert('<%= UnicodeLanguageUtil.get(pageContext, "Chưa có công trình nào đc chọn!") %>');
                            return false;
                    }
                    if (confirm('<%= UnicodeLanguageUtil.get(pageContext, "Có phân công lại các công trình được chọn không? ") %>')) {
...
Liferay.provide(
window,
'<portlet:namespace />huyCTTrongKeHoach',
 function() {
                    var checkBoxValue = Liferay.Util.listCheckedExcept(document.<portlet:namespace />fm, "<portlet:namespace />allRowIds");
                    if(checkBoxValue==""||checkBoxValue==null){
                            alert('<%= UnicodeLanguageUtil.get(pageContext, "Chưa có công trình nào đc chọn!") %>');
                    ...
                    if (confirm('<%= UnicodeLanguageUtil.get(pageContext, "Có huỷ bỏ các công trình được chọn không? ") %>')) {
                    document.<portlet:namespace />fm.<portlet:namespace />orgIds.value=checkBoxValue;
                    submitForm(document.<portlet:namespace />fm, "<%=phanCongLaiKeHoachKieDemCTHuyBoURL.toString()%>");
...
<liferay-portlet:renderURLParams varImpl="searchURL" />
		...
		<%@ include file="/html/portlet/kehoachkiemdemvesinh/include_kehoachkiemdemvesinh_detail_form.jspf"%>
			<liferay-ui:search-container-results>
				<%@ include file="/html/portlet/kehoachkiemdemvesinh/search_congtrinh_database_result.jspf"%>
			</liferay-ui:search-container-results>
			<liferay-ui:search-container-row className="vn.dtt.sol.ns.reporting.ThongTinKiemDemVeSinhCongTrinhModel" keyProperty="id" modelVar="kiemDemNuoc" >
				<liferay-portlet:renderURL varImpl="viewDetailThongTinUrl" >
					<portlet:param name="mvcPath" value="/html/portlet/kehoachkiemdemvesinh/view_detail_thong_tin_kiem_dem_cong_trinh.jsp" />
					<portlet:param name="<%=KeHoachKiemDemVeSinhTerms.ID %>" value="<%=String.valueOf(keHoachKiemDemVeSinh.getId()) %>" />
				...
				<liferay-ui:search-container-column-text href="<%=viewDetailThongTinUrl.toString() %>" name="<%= LanguageUtil.get(pageContext, \"dia-chi\") %>" value="<%= kiemDemNuoc.getDiaChi() %>" />
				<liferay-ui:search-container-column-text href="<%=viewDetailThongTinUrl.toString() %>" name="<%= LanguageUtil.get(pageContext, \"ten-cong-trinh\") %>" value="<%= kiemDemNuoc.getTenCongTrinh() %>" />
