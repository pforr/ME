		...
		<aui:form action="<%= searchURL %>" method="get" name="fm2">
			...
			<aui:fieldset>
				<liferay-ui:search-container delta="<%= 20 %>" emptyResultsMessage="no-result-were-found">
					...
					<liferay-ui:search-container-results results="<%=  DiaBanQuanLyUtils.getListTinhTU(user, permissionChecker.isOmniadmin() ? true : false, searchContainer.getStart(), searchContainer.getEnd()) %>"
										 total="<%= DiaBanQuanLyUtils.countListTinhTU(user, permissionChecker.isOmniadmin() ? true : false) %>">
					...
					>
						...
						<liferay-ui:search-container-column-text
							...
							cssClass="col-small"
						>
							<%= UserMappingBusiness.countUser(StringPool.BLANK, 2, tinh.getNode1(), StringPool.BLANK) %>
						</liferay-ui:search-container-column-text>
