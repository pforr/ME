...
public class HTCapNuocBenVungExport {
	...
	public static HTCapNuocBenVung getModel(String namBaoCao, String maTinh, String soBaoCao, String ngayBaoCao, String nguoiLap, String nguoiDuyet) {
		log.info("==START DLI 2.1 TINH==HTCapNuocBenVungExport==");
		
		String DANHGIABENVUNG = "1"; // 1 la da danh gia ben vung
		...
		List<TramCapNuoc> lstTramCapNuoc = TramCapBusiness.getTramCapNuocByDanhGiaBenVung(maTinh, namBaoCao, DANHGIABENVUNG);
		...
		if(Validator.isNotNull(lstTramCapNuoc) && lstTramCapNuoc.size() > 0){
			...
			for(TramCapNuoc tramCapNuoc : lstTramCapNuoc){
				...
				DanhGiaBenVung danhGiaBenVungSoNgHuongLoi = Validator.isNotNull(namBaoCao) ?
						DanhGiaBenVungBussiness.getDanhGiaBenVungByTramCapNuocAndNam(tramCapNuoc.getId(), namBaoCao, "1") : null;
				// ketquadanhgia = 1 : ben vung
		...
		log.info("==END DLI 2.1 TINH==HTCapNuocBenVungExport==");
		return result;
	}
}
